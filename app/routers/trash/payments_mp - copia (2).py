...
def _build_tickets_pdf_bytes(rows: list[dict]) -> bytes:
    """Genera un PDF "lindo" para tickets.

    - Branding TicketPro
    - Datos del evento
    - Grilla 2x2 (hasta 4 tickets por página). Si hay más, sigue en páginas siguientes.
    """
    buf = io.BytesIO()
    ...
    def draw_header():
        ...
        c.drawString(margin, page_h - header_h + 6 * mm, "TICKETPRO")
        c.setFont("Helvetica", 9)
        c.setFillColor(colors.Color(1, 1, 1, alpha=0.75))
        c.drawRightString(page_w - margin, page_h - header_h + 6 * mm, "Entradas oficiales • QR único")
        c.setFillColor(colors.black)
    ...
    usable_h = page_h - header_h - margin - margin
    usable_w = page_w - margin - margin
    card_w = (usable_w - gutter) / 2
    card_h = (usable_h - gutter) / 2

    for page_rows in chunk(rows, per_page):
        draw_header()

        for idx, r in enumerate(page_rows):
            col = idx % 2
            rowi = idx // 2  # 0..1 top to bottom

            x = margin + col * (card_w + gutter)
            y_top = page_h - header_h - margin - rowi * (card_h + gutter)
            y = y_top - card_h
            ...
            ticket_type = r.get("ticket_type") or "Entrada"
            qr_payload = r.get("qr_payload") or str(ticket_id)
            event_slug = r.get("event_slug")
            event_title = r.get("event_title") or event_slug or "Evento"
            ...
            c.drawString(x + pad, y + card_h - pad - 2 * mm, str(event_title)[:46])

            # Meta
            c.setFont("Helvetica", 9)
            meta_y = y + card_h - pad - 9 * mm
            if event_date or event_time:
                c.setFillColor(colors.Color(0, 0, 0, alpha=0.75))
                c.drawString(x + pad, meta_y, f"{event_date} {event_time}".strip())
                meta_y -= 5 * mm

            c.setFillColor(colors.Color(0, 0, 0, alpha=0.75))
            c.drawString(x + pad, meta_y, f"Tipo: {ticket_type}"[:60])
            meta_y -= 5 * mm
            c.setFillColor(colors.Color(0, 0, 0, alpha=0.65))
            c.drawString(x + pad, meta_y, f"Ticket: {ticket_id}"[:60])
            ...
            qr_size = min(card_w, card_h) * 0.55
            qr_x = x + card_w - pad - qr_size
            qr_y = y + pad + 6 * mm
            ...
            c.drawString(x + pad, y + pad, "Mostrá este QR en el ingreso. No lo publiques.")
            c.setFillColor(colors.black)
