...
def api_sale_items(
    ...
    with get_conn() as conn:
        rows = conn.execute(
            """
            SELECT id, event_slug, name, price_cents, stock, created_at
            FROM sale_items
            WHERE tenant_id = %s AND tenant = %s AND event_slug = %s
            ORDER BY id ASC
            """,
            (tenant_id, producer, event_slug),
        ).fetchall()
    items = []
    for r in rows:
        # rows can be dict-like or tuple-like depending on cursor; be defensive
...
def api_sale_item_create(
    ...
    """
    Crea un sale_item para un evento del productor logueado.
    """
    tenant_id = (payload.tenant_id or "default").strip() or "default"
    producer = user.get("producer")
    ...
    # En DB usamos "stock". Si el front manda "capacity" (legacy), lo tomamos como stock.
    stock = payload.stock if payload.stock is not None else payload.capacity

    with get_conn() as conn:
        row = conn.execute(
            """
            INSERT INTO sale_items (tenant_id, tenant, event_slug, name, price_cents, stock)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id, event_slug, name, price_cents, stock
            """,
            (tenant_id, producer, event_slug, name, price_cents, stock),
        ).fetchone()
        conn.commit()
    ...
    with get_conn() as conn:
        row = conn.execute(
            ...
            SET name = %s,
                price_cents = %s,
                stock = %s
            WHERE tenant_id = %s AND tenant = %s AND event_slug = %s AND id = %s
            RETURNING id, event_slug, name, price_cents, stock
            """,
            (
                (payload.name or "").strip(),
                int(payload.price_cents or 0),
                (payload.stock if payload.stock is not None else payload.capacity),
                tenant_id,
                producer,
                event_slug,
                sale_item_id,
...
def api_sale_item_delete(
    ...
    with get_conn() as conn:
        deleted = conn.execute(
            ...
            WHERE tenant_id = %s AND tenant = %s AND event_slug = %s AND id = %s
            """,
            (tenant_id, producer, event_slug, sale_item_id),
        ).rowcount
        conn.commit()
...
def api_list_sellers(
    ...
    with get_conn() as conn:
        rows = conn.execute(
            """
            SELECT id, event_slug, name, pin, created_at
            FROM sellers
            WHERE tenant_id = %s AND tenant = %s AND event_slug = %s
            ORDER BY id ASC
            """,
...
def api_seller_create(
    ...
    with get_conn() as conn:
        row = conn.execute(
            """
            INSERT INTO sellers (tenant_id, tenant, event_slug, name, pin)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id, event_slug, name, pin
            """,
            (tenant_id, producer, event_slug, name, pin),
        ).fetchone()
        conn.commit()
    d = dict(row) if not isinstance(row, dict) else row
    return {"ok": True, "seller": d}
...
def api_seller_delete(
    ...
    with get_conn() as conn:
        deleted = conn.execute(
            ...
            WHERE tenant_id = %s AND tenant = %s AND event_slug = %s AND id = %s
            """,
            (tenant_id, producer, event_slug, seller_id),
        ).rowcount
        conn.commit()
