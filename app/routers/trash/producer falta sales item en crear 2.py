...
class EventCreateIn(BaseModel):
    # Slug opcional (URL corta). Si no viene, se deriva del título.
...
# -------------------------------------------------------------------
# Upload flyer (Render Disk) + guarda URL en events.flyer_url
# -------------------------------------------------------------------
@router.post("/events/{slug}/flyer")
def api_event_upload_flyer(
    request: Request,
    slug: str,
    ...
    slug = (slug or "").strip()
    if not slug:
        raise HTTPException(status_code=400, detail="missing_slug")

    if not _can_edit_event(tenant_id, slug, producer):
        raise HTTPException(status_code=403, detail="forbidden_event")

    filename = (file.filename or "flyer").strip()
    ext = os.path.splitext(filename)[1].lower()
    if ext not in [".jpg", ".jpeg", ".png", ".webp"]:
        raise HTTPException(status_code=400, detail="invalid_file_type")

    base_dir = _uploads_dir()
    target_dir = _ensure_dir(os.path.join(base_dir, "events", tenant_id, slug))
    safe_name = f"flyer-{_now_epoch_s()}{ext}"
    target_path = os.path.join(target_dir, safe_name)

    with open(target_path, "wb") as out:
        shutil.copyfileobj(file.file, out)

    public_url = f"{_public_base_url(request)}/uploads/events/{tenant_id}/{slug}/{safe_name}"

    with get_conn() as conn:
        cols = _table_columns(conn, "events")
        if "flyer_url" in cols:
            conn.execute(
                "UPDATE events SET flyer_url=%s, updated_at=%s WHERE tenant_id=%s AND slug=%s",
                (public_url, _now_epoch_s(), tenant_id, slug),
            )
        elif "hero_bg" in cols:
            conn.execute(
                "UPDATE events SET hero_bg=%s, updated_at=%s WHERE tenant_id=%s AND slug=%s",
                (public_url, _now_epoch_s(), tenant_id, slug),
            )
        conn.commit()
