...
def api_producer_events(request: Request, user: dict = Depends(_require_auth)):
    ...
    # No usamos request.session["producer"] porque en algunos flujos puede ser un dict (usuario)
    # y eso rompe el match con la columna events.tenant. La fuente de verdad es el usuario autenticado.
    producer = (user or {}).get("producer") or _producer_from_request(request)
    tenant = producer
