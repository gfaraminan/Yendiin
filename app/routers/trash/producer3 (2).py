...
def api_list_sale_items(
    ...
    """Compat:
    - /api/producer/sale-items?event=slug   (viejo)
    - /api/producer/sale-items?event_slug=slug&tenant_id=default (nuevo)
    """
    tenant = (tenant_id or _tenant_from_request(request)).strip() or _tenant_from_request(request)
...
def api_seller_create(request: Request, payload: SellerUpsertIn):
    ...
    with get_conn() as conn:
        # ✅ si NO manda accept_terms, validamos que el evento exista (y listo)
        if not getattr(payload, "accept_terms", False):
            cur = conn.execute(
                "SELECT 1 FROM events WHERE tenant_id=%s AND slug=%s LIMIT 1",
                (tenant, event_slug),
            )
