...
def api_list_sale_items(request: Request, event: str = "", event_slug: str = ""):
    """Compat: /api/producer/sale-items?event=slug"""
    tenant = _tenant_from_request(request)
    event_slug = (event or event_slug or "").strip()
    if not event_slug:
        return []
...
# -------------------------------------------------------------------
@router.get("/sellers")
def api_list_sellers(request: Request, event: str = "", event_slug: str = ""):
    tenant = _tenant_from_request(request)
    event_slug = (event or event_slug or "").strip()
    if not event_slug:
        return []
