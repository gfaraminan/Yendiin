...
def producer_dashboard(tenant_id: str, event_slug: str):
    """Resumen mínimo para el panel productor (evita 404 y habilita click en evento)."""
    with get_conn() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT id, slug, title, date_text, city, venue, flyer_url, status
                FROM events
            ...
            cur.execute(
                """
                SELECT id, name, kind, price_cents, stock_total, stock_sold, active
                FROM sale_items
            ...
            try:
                cur.execute(
                    ...
                    WHERE o.tenant_id=%s AND o.event_slug=%s AND o.status IN ('paid','completed')
                    """,
                    (tenant_id, event_slug),
                )
                r = cur.fetchone() or {}
            ...
            return {
                ...
                "event": ev,
                "sale_items": items,
                "metrics": {"sold": sold, "revenue_cents": revenue_cents},
            }
@router.post("/events")
