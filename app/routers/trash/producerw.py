...
async def api_sale_item_create(request: Request, payload: SaleItemUpsertIn) -> Any:
    ...
    # We'll include tenant_id only if the column exists in the table.
    with db_conn() as db:
        col_types = _table_column_types(db, "sale_items")
        ...
        vals = [tenant, event_slug, payload.name, payload.price_cents, True, bool(payload.is_ticket), payload.stock_total, payload.stock_total]
        if has_tid:
