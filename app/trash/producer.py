...
def _producer_from_request(request: Request) -> str:
    ...
    # Opción A: si hay usuario autenticado, el "owner" debe salir del user de la sesión.
    # Esto evita el bug clásico: create con header x-producer y update sin header => 404.
    p: Any = None
    if hasattr(request, "session"):
        su = request.session.get("user")
...
def _event_update_impl(request: Request, slug: str, payload: EventUpdateIn):
    ...
    with get_conn() as conn:
        ...
        # 1) Load the event by tenant+slug (do NOT depend on x-producer header).
        cur0 = conn.execute(
        ...
        # 2) Ownership check (Option A).
        # Different DB versions used different column names; we try a few.
        r0 = dict(row0) if hasattr(row0, "keys") else row0
        owner = None
        if isinstance(r0, dict):
            for k in ("producer_id", "owner_id", "owner", "producer", "tenant"):
                v = r0.get(k)
                if isinstance(v, str) and v.strip():
                    owner = _norm_id(v)
                    break
        if owner and owner != _norm_id(producer):
        ...
        # psycopg rows can be tuple-like; fallback gracefully
        slug_out = None
        if hasattr(row, "keys"):
            try:
                slug_out = row["slug"]
        ...
        return {"ok": True, "updated": True, "slug": slug_out}
...
def api_sale_item_create_alias(request: Request, payload: Dict[str, Any], user=Depends(_require_auth)):
    """
    Alias REST: some frontends POST /sale-items instead of /sale-items/create
    """
    return api_sale_item_create(request, payload, user)
