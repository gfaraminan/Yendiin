...
function buildUberLink(ev) {
  const lat = Number(ev?.lat);
  const lng = Number(ev?.lng);
  if (Number.isFinite(lat) && Number.isFinite(lng)) {
    return `https://m.uber.com/ul/?action=setPickup&dropoff[latitude]=${encodeURIComponent(String(lat))}&dropoff[longitude]=${encodeURIComponent(String(lng))}`;
  }
  const fallback = String(ev?.address || ev?.venue || ev?.city || "").trim();
  if (!fallback) return "";
  return `https://m.uber.com/ul/?action=setPickup&dropoff[formatted_address]=${encodeURIComponent(fallback)}`;
}
    ...
    try {
      window.google.accounts.id.prompt((notification) => {
        if (notification?.isNotDisplayed?.() || notification?.isSkippedMoment?.()) {
          setGooglePromptError("No pudimos abrir el selector de Google en este navegador. Probá habilitar el inicio de sesión de terceros para este sitio.");
        }
      });
    } catch (e) {
      console.error(e);
      setGooglePromptError("No se pudo abrir Google en este navegador.");
    }
  };
          ...
          {loginMethod === "google" && (
            <>
              <button
                ...
                disabled={!googleClientId || !ready}
                className="w-full min-h-[44px] rounded-full bg-white/10 hover:bg-white/15 border border-white/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-3 px-5"
              >
                <span className="w-8 h-8 rounded-full bg-white text-[#4285F4] text-lg font-black flex items-center justify-center">G</span>
                <span className="text-[14px] font-semibold leading-none text-white/90">Continuar con Google</span>
              </button>
                ...
                <div className="flex items-start justify-between gap-6 mb-8">
                  ...
                  >
	                ...
	                <div className="space-y-4">
	                  {(producerEvents || []).map((ev) => (
                    (() => {
                      const totalTickets = Number(ev.stock_total || 0);
                      const soldTickets = Number(ev.stock_sold || 0);
                      const soldPercent = totalTickets > 0
                        ? Math.max(0, Math.min(100, Math.round((soldTickets / totalTickets) * 100)))
                        : 0;
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className={`flex-1 md:flex-none px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all text-white no-underline ${UI.button}`}
                        >
              ...
              <div className={`p-8 rounded-[2.5rem] ${UI.card}`}>
                ...
                <div className="space-y-3">
                  <button className="w-full py-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                    <SafeMailIcon size={16} /> Enviar campaña (demo)
                  </button>
                  <button className="w-full py-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                  ...
                  <button className="w-full py-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                    <SafeSocialIcon size={16} /> Publicar (demo)
                  </button>
                </div>
