...
const EditorModal = ({
  ...
  const [sellerDraft, setSellerDraft] = useState({
    code: "",
    name: "",
    // backend lo pide en algunos casos (retorna 'terms_required' si falta)
    terms_required: true,
  });
  ...
  const createSaleItem = async () => {
    ...
    setTabError("");
    try {
      const name = (saleItemDraft.name || "").trim();
      const price = Number(saleItemDraft.price);
      const sort = Number.isFinite(Number(saleItemDraft.sort_order))
        ? parseInt(String(saleItemDraft.sort_order), 10)
        : 0;
      ...
      setSaleItemDraft({ name: "GENERAL", price: 10, sort_order: 1 });
      await loadSaleItems();
    } catch (e) {
  ...
  const createSeller = async () => {
    ...
    try {
      ...
      // El backend parece exigir 'terms_required' y, si está en true, texto/URL
      const payload = {
        tenant_id: tenantId,
        event_slug: eventSlug,
        code,
        name,
        terms_required: true,
        terms_text: "El vendedor acepta los términos y condiciones del evento.",
        terms_url: "",
      };
      ...
      setSellerDraft({ code: "", name: "", terms_required: true });
      await loadSellers();
    } catch (e) {
      setTabError(String(e?.message || e));
    } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {events.map((ev) => (
                <button
                  ...
                  onClick={() => {
                    setSelectedEvent(ev);
                    setSelectedTicket(ev.items?.[0] || null);
                    setQuantity(1);
