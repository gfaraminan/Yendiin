...
// -------------------------
// Fecha helper (evita corrimiento por zona horaria)
// - Si date_text es "YYYY-MM-DD" lo formatea usando UTC para que NO cambie el día
// - Si date_text ya es texto (ej: "Sáb, 01 de Enero") lo devuelve tal cual
// -------------------------
function formatEventDateText(ev) {
  const raw =
    (ev?.date_text ?? ev?.date ?? ev?.starts_at ?? ev?.startsAt ?? ev?.start_date ?? "")
      .toString()
      .trim();

  if (!raw) return "";

  // Si ya viene "humano", no lo tocamos
  if (/[a-zA-ZáéíóúñÁÉÍÓÚÑ]/.test(raw) && !/^\d{4}-\d{2}-\d{2}/.test(raw)) return raw;
            ...
            <div className="relative h-40">
              ...
              <div className="absolute bottom-0 left-0 p-4">
                ...
                <div className="text-[11px] text-neutral-300 mt-2 flex items-center gap-2">
                  <Calendar size={14} /> {formatEventDateText(ev)}
                </div>
              </div>
            </div>
