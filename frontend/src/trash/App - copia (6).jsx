...
const EditorModal = ({
  ...
  const createSaleItem = async () => {
    ...
    try {
      ...
      await fetchJson(`/api/producer/sale-items/create?tenant_id=${encodeURIComponent(tenantId)}&event_slug=${encodeURIComponent(eventSlug)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
// -------------------------
  ...
  const saveEvent = async (finalize = true, closeOnSuccess = true) => {
    ...
    try {
      ...
      // Si es creación y hay items locales (fallback por 403 cuando el evento aún no existía),
      // los sincronizamos ahora que ya tenemos slug real del backend.
      const createdSlug = (data && data.slug) ? data.slug : (payload.slug || "");
      if (method === "POST" && createdSlug) {
        if (createdSlug !== eventSlug) setEventSlug(createdSlug);

        const pending = (saleItems || []).filter(si => si && si._localOnly);
        if (pending.length) {
          for (const si of pending) {
            try {
              const payloadSI = {
                ...
                kind: si.kind || "ticket",
                name: si.name || si.title || "Item",
                price: Number(si.price || 0),
                price_cents: Number.isFinite(Number(si.price_cents)) ? Number(si.price_cents) : Math.round(Number(si.price || 0) * 100),
                stock_total: Number.isFinite(Number(si.stock_total)) ? Number(si.stock_total) : Number(si.stock || 0),
                currency: si.currency || "ARS",
                description: si.description || "",
                active: si.active !== false,
              };
              const rSI = await fetch(`/api/producer/sale-items/create?tenant_id=${encodeURIComponent(tenantId || 'default')}&event_slug=${encodeURIComponent(createdSlug)}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify(payloadSI),
              });
              const dSI = await readJsonOrText(rSI);
              if (rSI.ok) {
                const newId = (dSI && dSI.id) ? dSI.id : si.id;
                setSaleItems(prev => (prev || []).map(x => (x === si || x._localId === si._localId) ? { ...x, id: newId, _localOnly: false } : x));
              } else {
                console.warn("sale-item sync failed", dSI);
              }
            } catch (e) {
              console.warn("sale-item sync error", e);
            }
          }
          try { await loadSaleItems(); } catch {}
        }
      }
