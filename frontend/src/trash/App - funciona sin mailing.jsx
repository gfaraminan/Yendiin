...
const normalizeAssetUrl = (u) => {
  ...
  if (!s) return "";
  // No persistimos ni mostramos blob: como URL "final"
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="aspect-[16/9] w-full rounded-2xl overflow-hidden bg-black/30 border border-white/10 flex items-center justify-center">
        {(normalizeAssetUrl(flyerPreview) || normalizeAssetUrl(editFormData.flyer_url)) ? (
          <img
            src={normalizeAssetUrl(flyerPreview) || normalizeAssetUrl(editFormData.flyer_url) || FALLBACK_FLYER}
            alt="Flyer preview"
...
try {
  if (typeof flyerFilePending !== "undefined" && flyerFilePending && createdSlug) {
    const urlFlyer = await uploadFlyerForEvent(createdSlug, flyerFilePending);
    // Persistimos en el form (ya está actualizado en DB por el endpoint)
    setEditFormData((p) => (p ? { ...p, flyer_url: urlFlyer } : p));
    if (typeof setFlyerFilePending === "function") setFlyerFilePending(null);
    if (typeof setFlyerPreview === "function") setFlyerPreview(urlFlyer);
  }
} catch (e) {
  console.error(e);
