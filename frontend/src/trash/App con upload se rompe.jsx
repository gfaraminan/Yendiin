...
const EditorModal = ({
  ...
  const onPickFlyerFile = async (file) => {
    ...
    // Upload real (Render Disk / backend). Requiere login de productor.
    try {
      ...
      // Convertimos a URL absoluta para que funcione en cualquier lugar (móvil/desktop).
      const absoluteUrl = data.url.startsWith("http")
        ? data.url
        : `${window.location.origin}${data.url.startsWith("/") ? "" : "/"}${data.url}`;

      // Persistimos en el draft/evento (esto es lo que termina en Postgres via saveEvent)
      setEditFormData((p) => ({ ...p, flyer_url: absoluteUrl }));
      setFlyerPreview(absoluteUrl);

      // Limpieza del blob local
      setTimeout(() => {
        try { URL.revokeObjectURL(localUrl); } catch {}
      }, 1500);
    } catch (e) {
      console.error(e);
      // Si falla el upload, dejamos la preview local para que no frene el flujo,
      // pero avisamos que no va a persistir.
      alert("No se pudo subir el flyer. Quedó solo en preview local (se pierde al refrescar).");
      setEditFormData((p) => ({ ...p, flyer_url: localUrl }));
    }
  };
...
{wizardStep === 1 && (
  <div className="space-y-6">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <div className="text-[11px] text-white/45 mt-2">
          Se sube al backend y queda guardada en el Disk de Render. La URL se guarda en el evento.
        </div>
        ...
        <div className="mt-4">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setEditFormData((p) => ({ ...p, flyer_url: e.target.value }));
            }}
            placeholder="https://..."
          />
        </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="text-sm font-semibold mb-3">Ubicación</div>
