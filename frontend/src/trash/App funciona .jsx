...
const EditorModal = ({
  ...
  setFlyerPreview(url);
  // No pisamos flyer_url con un blob: la URL real la devuelve el backend al subir el archivo.
};
...
// -------------------------
// Flyer upload (producer)
// -------------------------
const uploadFlyer = async (slug, file) => {
  ...
  const data = await readJsonOrText(res);
  if (!res.ok || !data?.ok) {
    throw new Error(data?.detail || "Error subiendo flyer");
  }
  return data;
};
...
<div className="mt-2 flex items-center gap-3">
  <input
    type="file"
    accept="image/*"
    className="block w-full text-xs text-white/70 file:mr-4 file:rounded-xl file:border-0 file:bg-white/10 file:px-4 file:py-2 file:text-xs file:font-semibold file:text-white hover:file:bg-white/15"
    onChange={(e) => onPickFlyerFile(e.target.files?.[0])}
  />
  {flyerPreview ? (
    <span className="text-[11px] text-white/50">preview listo</span>
  ) : null}
</div>
...
// -------------------------
  ...
  const saveEvent = async (finalize = true, closeOnSuccess = true) => {
    ...
    try {
      ...
      // Si hay un archivo seleccionado, lo subimos y actualizamos flyer_url
      if (flyerFile) {
        const up = await uploadFlyer(editFormData.slug, flyerFile);
        const newUrl = up?.flyer_url || up?.url || up?.path;
        if (newUrl) {
          try {
            // persistimos la url en el evento para que se vea en público
            await fetch(`/api/producer/events/${editFormData.slug}`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              credentials: "include",
              body: JSON.stringify({ flyer_url: newUrl }),
            });
          } catch (e) {
            // no bloquea
          }
          setEditFormData((p) => ({ ...p, flyer_url: newUrl }));
        }
        setFlyerFile(null);
      }
