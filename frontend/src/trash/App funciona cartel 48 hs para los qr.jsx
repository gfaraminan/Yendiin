...
export default function App() {
  ...
  const [mpAssetsForOrder, setMpAssetsForOrder] = useState([]);
  const [mpAssetsReady, setMpAssetsReady] = useState(false);
  const [mpAssetsWaitMsg, setMpAssetsWaitMsg] = useState(null);
  const mpWaitStartedAtRef = useRef(null);
...
const [selectedTicket, setSelectedTicket] = useState(null);
  ...
  useEffect(() => {
    ...
    // Entramos a la pantalla de espera: reseteamos estado local
    setMpAssetsReady(false);
    setMpAssetsForOrder([]);
    setMpAssetsWaitMsg(null);
    mpWaitStartedAtRef.current = Date.now();

    let tries = 0;
    const maxTries = 60; // ~120s (2s * 60)
    const intervalMs = 2000;
    let stopped = false;

    const normalizeId = (v) => (v == null ? "" : String(v));

    const pickAssetsForOrder = (assets) => {
      const oid = normalizeId(orderId);
      if (!Array.isArray(assets) || !oid) return [];

      // Intento 1: match directo por order_id / orderId
      const direct = assets.filter((a) => {
        const aOid =
          normalizeId(a?.order_id) ||
          normalizeId(a?.orderId) ||
          normalizeId(a?.order?.id) ||
          normalizeId(a?.order?.order_id);
        return aOid && aOid === oid;
      });
      if (direct.length) return direct;

      // Intento 2: algunos backends guardan el order_id en metadata
      const meta = assets.filter((a) => {
        const mOid =
          normalizeId(a?.meta?.order_id) ||
          normalizeId(a?.meta?.orderId) ||
          normalizeId(a?.metadata?.order_id) ||
          normalizeId(a?.metadata?.orderId);
        return mOid && mOid === oid;
      });
    ...
    const tick = async () => {
      ...
      try {
        const res = await fetch(`${API}/api/orders/my-assets?tenant=${encodeURIComponent(tenant)}`, {
          method: "GET",
          credentials: "include",
        ...
        if (res.status === 401) {
          // Todavía no hay sesión (o se perdió). Seguimos mostrando espera.
          if (tries === 1) setMpAssetsWaitMsg("Iniciando sesión…");
          return;
        }

        const data = await res.json().catch(() => null);
        const assets = data?.assets || [];
        // Mantener el estado general por si otras vistas lo usan
        if (Array.isArray(assets)) setMyAssets(assets);

        const picked = pickAssetsForOrder(assets);
        if (picked.length) {
          setMpAssetsForOrder(picked);
          setMpAssetsReady(true);
          setMpAssetsWaitMsg(null);

          // Limpia el hash para que un refresh no re-dispare el checkout/success
          try {
        ...
        // Mensaje amigable mientras esperamos
        if (tries === 1) setMpAssetsWaitMsg("Confirmando tu pago…");
        else if (tries === 4) setMpAssetsWaitMsg("Estamos emitiendo tus QR…");
        else if (tries === 12) setMpAssetsWaitMsg("Últimos detalles… (a veces tarda unos segundos)");
      } catch (e) {
        // No frenamos el loop por errores transitorios
        if (tries === 1) setMpAssetsWaitMsg("Conectando…");
      }

      if (tries >= maxTries) {
        stopped = true;
        setMpAssetsWaitMsg("Tu pago fue confirmado, pero los QR todavía no aparecen. Tocá “Reintentar” o miralos luego en Mis Tickets.");
      }
    };
    ...
    return () => {
      stopped = true;
      clearInterval(t);
      mpWaitStartedAtRef.current = null;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        {view === "mpWait" && (
          <div className="pt-0 pb-20 px-6 max-w-7xl mx-auto animate-in fade-in text-white">
            <div className="max-w-3xl mx-auto">
              <div
                className={`p-10 rounded-[32px] border border-white/10 bg-white/[0.04] shadow-[0_20px_80px_rgba(0,0,0,0.6)]`}
              >
                {!mpAssetsReady ? (
                  <>
                    <div className="text-xs tracking-[0.25em] text-white/40 text-center mb-3">
                    ...
                    <div className="mt-4 text-white/70 text-center">
                      Mercado Pago confirmó la operación, pero estamos esperando la validación final y la emisión de tus QR.
                      
                    </div>

                    <div className="mt-8 flex items-center justify-center gap-3 text-white/70">
                      <span className="inline-block w-4 h-4 rounded-full border-2 border-white/30 border-t-white/80 animate-spin" />
                      <span className="text-sm">
                        {mpAssetsWaitMsg || `Procesando… Orden ${String(mpReturn?.order_id || "").slice(0, 8)}…`}
                      </span>
                    </div>
                    ...
                    <div className="mt-8 text-center text-xs text-white/40">
                      Si se demora, tocá “Actualizar”. Tus entradas también quedarán disponibles luego en “Mis Tickets”.
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <h2 className="text-4xl md:text-5xl font-extrabold text-center leading-tight">
                      ¡Listo! Acá están tus QR
                    </h2>

                    <div className="mt-4 text-white/70 text-center">
                      Guardalos o descargá el PDF. También podés verlos después en “Mis Tickets”.
                    </div>

                    <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {mpAssetsForOrder.map((a, idx) => {
                        const qrUrl =
                          a?.qr_url ||
                          a?.qrUrl ||
                          a?.qr ||
                        ...
                        const pdfUrl = a?.pdf_url || a?.pdfUrl || a?.pdf;

                        const label = a?.label || a?.name || a?.title || `Entrada ${idx + 1}`;
                        const code = a?.code || a?.ticket_code || a?.ticketCode || a?.id;

                        return (
                          <div key={a?.id || a?.asset_id || idx} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                            <div className="flex items-start justify-between gap-4">
                              <div>
                                <div className="font-semibold">{label}</div>
                                {code ? <div className="text-xs text-white/50 mt-1">Código: {String(code)}</div> : null}
                              </div>
                              ...
                              {pdfUrl ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-xs px-3 py-2 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 transition"
                                >
                                  Descargar PDF
                                </a>
                              ) : null}
                            ...
                            {qrUrl ? (
                              <div className="mt-4 flex items-center justify-center">
                                <img
                                  src={qrUrl}
                                  alt={label}
                                  className="w-56 max-w-full rounded-xl border border-white/10 bg-black/20"
                                  loading="lazy"
                                />
                              </div>
                            ) : (
                              <div className="mt-4 text-sm text-white/60">
                                QR no disponible todavía para este ítem. Probá “Actualizar”.
                              </div>
                            )}
                          </div>
                        );
