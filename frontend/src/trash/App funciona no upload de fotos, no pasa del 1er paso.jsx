...
const EditorModal = ({
  ...
  // Crea el evento en backend (si es nuevo) para que los sale-items no fallen con 403.
  const ensureEventCreated = async () => {
    try {
      if (!editFormData) return false;
      if (editFormData?._is_new === false) return true;
      ...
      const r = await fetch("/api/producer/events", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      const data = await readJsonOrText(r);
      if (!r.ok || !data?.ok) throw new Error(data?.detail || "No se pudo crear el evento");

      const createdSlug = data.slug || payload.slug;
      setEditFormData((prev) => ({ ...(prev || {}), slug: createdSlug, _is_new: false }));

      try { await loadProducerEvents(); } catch(e) {}
      try { await refreshPublicEvents(); } catch(e) {}
      return true;
    } catch (e) {
      console.error(e);
      alert("Error creando evento (paso 1): " + (e?.message || e));
      return false;
    }
  };
...
const canGoNext = () => getStepErrors(wizardStep).length === 0;
  ...
  const uploadFlyerForCurrentEvent = async (file) => {
    try {
      if (!file) return;
      ...
      const r = await fetch(`/api/producer/events/${encodeURIComponent(slug)}/flyer?${qs.toString()}`, {
        method: "POST",
        credentials: "include",
        body: fd,
      });
      const data = await readJsonOrText(r);
      if (!r.ok || !data?.ok) throw new Error(data?.detail || "No se pudo subir el flyer");
      const url = data.url;
      setEditFormData((prev) => ({ ...(prev || {}), flyer_url: url }));
      setFlyerPreview(url);
      try { await loadProducerEvents(); } catch(e) {}
      try { await refreshPublicEvents(); } catch(e) {}
    } catch (e) {
      console.error(e);
      alert("Error subiendo flyer: " + (e?.message || e));
    }
  };
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (f) uploadFlyerForCurrentEvent(f);
                        }}
                        className="block w-full text-xs text-white/70 file:mr-3 file:rounded-xl file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-white hover:file:bg-white/20"
                      />
                    </div>
