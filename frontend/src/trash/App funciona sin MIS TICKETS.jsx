...
const EditorModal = ({
  ...
  const loadSaleItems = async () => {
    if (!eventSlug) return;
    if (eventSlug === "nuevo-evento") {
      setTabError("Primero guardá el evento (todavía no existe en el backend).");
      return;
    }
    setTabBusy(true);
    setTabError("");
                          ...
                          <div className="space-y-2">
                            {Array.isArray(sellers) && sellers.map((s) => (
                              <div key={s.id} className="flex items-center justify-between gap-3 rounded-xl bg-black/20 border border-white/10 px-3 py-2">
                                <div className="min-w-0">
                                  <div className="text-sm font-semibold truncate">{s.name ?? "(sin nombre)"}</div>
...
export default function App() {
  ...
  const closeLoginModal = async () => {
    ...
    // Si el login fue disparado desde el botón "Productor", queremos
    // aterrizar directo en el panel del productor (no en la cartelera).
    if (postLoginTarget === "producer") {
      setPostLoginTarget(null);
      setView("producer");
      try {
        await loadProducerEvents();
      } catch (e) {
        // No rompemos la UI si falla el fetch.
        console.error("loadProducerEvents failed", e);
      }
    }
...
// -------------------------
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      const savedSlug = data?.event?.slug || data?.slug || payload?.slug;
      if (savedSlug) setSelectedProducerEventSlug(savedSlug);
      setIsEditing(false);
      setEditFormData(null);
    } catch (e) {
      console.error(e);
