...
// Fallback flyer (si el evento no tiene imagen o falla al cargar)
const FALLBACK_FLYER = `data:image/svg+xml;utf8,${encodeURIComponent(`
<svg xmlns='http://www.w3.org/2000/svg' width='1200' height='800'>
  <defs>
    <radialGradient id='g' cx='30%' cy='20%' r='90%'>
      <stop offset='0%' stop-color='#4f46e5' stop-opacity='0.55'/>
      <stop offset='55%' stop-color='#0b0b12' stop-opacity='1'/>
  ...
  <rect x='70' y='70' width='1060' height='660' rx='60' fill='url(#s)' stroke='rgba(255,255,255,0.08)'/>
  <g fill='rgba(255,255,255,0.85)'>
    <text x='120' y='240' font-family='Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial' font-size='56' font-weight='900' letter-spacing='2'>TICKETERA</text>
    <text x='120' y='315' font-family='Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial' font-size='84' font-style='italic' font-weight='900' letter-spacing='-1'>TicketPro</text>
  </g>
  <g fill='rgba(255,255,255,0.55)'>
    <text x='120' y='395' font-family='Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial' font-size='24' font-weight='800' letter-spacing='6'>EVENTO · IMAGEN NO DISPONIBLE</text>
    <text x='120' y='455' font-family='Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial' font-size='18' font-weight='600' letter-spacing='1'>Cargá tu flyer en Producer para personalizarlo.</text>
  </g>
  <g opacity='0.25'>
    <circle cx='940' cy='270' r='120' fill='rgba(79,70,229,0.9)'/>
    <circle cx='1020' cy='340' r='170' fill='rgba(79,70,229,0.45)'/>
    <circle cx='900' cy='390' r='210' fill='rgba(255,255,255,0.08)'/>
  </g>
</svg>
`)}`;

const getFlyerSrc = (ev) => {
  const src = ev?.flyer_url || ev?.hero_bg || "";
  return src?.trim() ? src : FALLBACK_FLYER;
};

// Precio "Desde" real: mínimo > 0 dentro de items. Si no hay, devolvemos null.
const getFromPrice = (ev) => {
  const prices = (ev?.items || [])
        ...
        <div className="flex gap-2 mb-8">
          {[
            ...
            <button
              ...
              className={`px-4 md:px-5 py-2.5 md:py-3 rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${
...
export default function App() {
  ...
  const FeaturedCarousel = ({ list }) => {
    ...
    return (
      <div className="mb-10">
        <style>{`.tp-scroll{scrollbar-width:none;-ms-overflow-style:none;}
.tp-scroll::-webkit-scrollbar{display:none;}`}</style>
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            <div className="text-[9px] font-black uppercase tracking-widest text-neutral-500">
              Destacados
            </div>
            <div className="text-xl font-black uppercase italic">Eventos recomendados</div>
          </div>
        ...
        <div
          ref={ref}
          className="tp-scroll flex gap-4 md:gap-6 overflow-x-auto scroll-smooth snap-x snap-mandatory overscroll-x-contain px-1 md:px-0"
        >
          {featured.map((ev) => {
            const from = getFromPrice(ev);
            return (
              <button
                key={ev.id}
                onClick={() => {
                ...
                className={`flex-none w-[82vw] sm:w-[340px] md:w-[360px] snap-start text-left overflow-hidden rounded-[2.5rem] ${UI.card} hover:border-indigo-600/40 transition-all`}
              >
                <div className="relative h-48">
                  ...
                  <div className="absolute bottom-0 left-0 p-6">
                    ...
                    <div className="text-2xl font-black uppercase italic mt-1 truncate">
                      {ev.title}
                    </div>
                ...
                <div className="p-6 flex items-center justify-between gap-4">
                  <div>
                    <div className="text-[9px] font-black uppercase tracking-widest text-neutral-500">Desde</div>
                    <div className="text-xl font-black text-indigo-400 italic">
                      {from ? formatMoney(from) : "Consultar"}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[9px] font-black uppercase tracking-widest text-neutral-500">Stock</div>
  ...
  const Header = () => {
    return (
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-5 flex items-center justify-between gap-4 md:gap-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-indigo-600 flex items-center justify-center shadow-[0_0_40px_rgba(79,70,229,0.45)]">
            ...
            <div>
              ...
              <div className="text-white font-black uppercase italic tracking-tight text-2xl md:text-3xl leading-none">
  ...
  return (
    <div className={`min-h-screen ${UI.bg} text-white overflow-x-hidden`}>
      ...
      <main className="min-h-screen">
        {/* PUBLIC */}
        {view === "public" && (
          <div className="pt-28 md:pt-32 pb-16 md:pb-20 px-4 md:px-6 max-w-7xl mx-auto animate-in fade-in text-white">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
              <div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black uppercase italic tracking-tight">
              ...
              <button
                onClick={() => setView("producer")}
                className={`w-full md:w-auto px-6 md:px-8 py-3.5 md:py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest text-white transition-all ${UI.button}`}
              >
                <ShieldCheck size={16} /> Ir a Producer
              </button>
            </div>

            {/* Carrusel de destacados (por ahora: los primeros N eventos) */}
            <FeaturedCarousel list={events} />
