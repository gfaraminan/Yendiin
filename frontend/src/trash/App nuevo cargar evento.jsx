...
const EditorModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
      <div className={`w-full max-w-5xl p-8 rounded-[2.5rem] ${UI.card} text-white`}>
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            <div className="text-[9px] font-black uppercase tracking-widest text-neutral-500">
              Editor (demo)
            ...
            <div className="text-[11px] text-neutral-400 mt-2">
              Flujo paso a paso: Evento → Tickets → Vendedores → Confirmación.
            </div>
          </div>
        ...
        {/* Stepper */}
        {(() => {
          const persisted = !!eventSlug && editFormData?._is_new === false;

          const steps = [
            { id: "info", label: "Evento", enabled: true },
            { id: "tickets", label: "Tickets", enabled: persisted },
            { id: "sellers", label: "Vendedores", enabled: persisted },
            { id: "confirm", label: "Confirmación", enabled: persisted },
          ];
              ...
              {steps.map((t, idx) => {
                ...
                return (
                  <button
                    ...
                    title={locked ? "Primero guardá el evento" : ""}
                  >
        ...
        {activeTab === "info" && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <label className="text-sm">
                ...
                <div className="mt-1 text-[11px] text-white/40">
                  Se usa para links y para cargar tickets/vendedores.
                </div>
              </label>
              ...
              <label className="text-sm">
                ...
                <input
                  type="date"
                  className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2"
                  value={(editFormData.date_iso || "").slice(0, 10)}
                  onChange={(e) => {
                    const v = e.target.value;
                    setEditFormData({ ...editFormData, date_iso: v, date_text: v });
                  }}
                />
                <div className="mt-1 text-[11px] text-white/40">
                  (Guardamos date_text también para compatibilidad.)
                </div>
              </label>
              ...
              <label className="text-sm md:col-span-2">
                <div className="text-white/60 mb-1">Venue / Dirección</div>
                <input
                  className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2"
                  value={editFormData.venue || ""}
                  onChange={(e) => setEditFormData({ ...editFormData, venue: e.target.value, address: e.target.value })}
                />
              </label>
              ...
              <div className="text-sm md:col-span-2 rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div>
                    <div className="font-semibold text-white">Ubicación (coords para Uber)</div>
                    <div className="text-xs text-white/60">
                      Podés setear lat/lng manual o usar tu ubicación actual (para pruebas).
                    </div>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      navigator.geolocation.getCurrentPosition(
                        (pos) => {
                          ...
                          setEditFormData((prev) => ({ ...prev, lat, lng }));
                        },
                        (err) => alert("No se pudo obtener la ubicación: " + (err?.message || err))
                      );
                    }}
                    className="px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest"
                  >
                    Usar mi ubicación
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <label className="text-sm">
                    <div className="text-white/60 mb-1">Latitud</div>
                    <input
                      className="w-full rounded-xl bg-black/20 border border-white/10 px-3 py-2"
                      value={editFormData.lat ?? ""}
                      onChange={(e) => setEditFormData({ ...editFormData, lat: e.target.value === "" ? "" : Number(e.target.value) })}
                      placeholder="-32.889..."
                    />
                  </label>
                  <label className="text-sm">
                    <div className="text-white/60 mb-1">Longitud</div>
                    <input
                      className="w-full rounded-xl bg-black/20 border border-white/10 px-3 py-2"
                      value={editFormData.lng ?? ""}
                      onChange={(e) => setEditFormData({ ...editFormData, lng: e.target.value === "" ? "" : Number(e.target.value) })}
                      placeholder="-68.845..."
                    />
                  </label>
                </div>
              ...
              <label className="text-sm md:col-span-2">
                <div className="text-white/60 mb-1">Flyer (subir imagen)</div>
                <input
                  type="file"
                  accept="image/*"
                  className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2"
                  onChange={(e) => {
                    ...
                    // (Demo) guardamos un blob URL para preview.
                    // En backend real: se sube a storage y se guarda URL pública.
                    setEditFormData((prev) => ({ ...prev, flyer_url: url, flyer_file_name: f.name }));
                  }}
                />
                {editFormData.flyer_url ? (
                  <div className="mt-3 flex items-center gap-3">
                    <img
                      src={editFormData.flyer_url}
                      alt="flyer preview"
                    ...
                    <div className="text-xs text-white/60 break-all">
                      {editFormData.flyer_file_name ? (
                        <>Archivo: <span className="text-white">{editFormData.flyer_file_name}</span></>
                      ) : (
                        <>Flyer: <span className="text-white">{editFormData.flyer_url}</span></>
                      )}
                      <div className="text-[11px] text-white/40 mt-1">
                        (Si preferís URL directa, dejame y te habilito un campo “Avanzado”.)
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="mt-1 text-[11px] text-white/40">
                    Subí una imagen para el flyer (recomendado).
                  </div>
                )}
              </label>

              <label className="text-sm md:col-span-2">
                <div className="text-white/60 mb-1">Alias de cobro</div>
                <input
                  className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2"
                  value={editFormData.payout_alias || ""}
                  onChange={(e) => setEditFormData({ ...editFormData, payout_alias: e.target.value })}
                  placeholder="Ej: ticketpro.mp / CBU / alias"
                />
              </label>
            ...
            <div className="mt-8 flex flex-col md:flex-row gap-3">
              <button
                onClick={async () => {
                  // Guardamos borrador para destrabar Tickets/Vendedores
                  await saveEvent({ closeAfterSave: false, finalize: false });
                  // al guardar, el saveEvent setea _is_new=false si salió bien
        ...
        {/* PASO 2/3: Tickets o Vendedores */}
        {(activeTab === "tickets" || activeTab === "sellers") && (
          <div className="mt-3">
            {!(eventSlug && editFormData?._is_new === false) ? (
              <div className="text-sm text-amber-200/90 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
                Primero guardá el evento (paso 1) para poder cargar {activeTab === "tickets" ? "tickets" : "vendedores"}.
              </div>
            ) : (
              <>
                {tabError && (
                ...
                {activeTab === "sellers" && (
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
                      <div className="text-sm text-white/70 mb-2">Agregar vendedor (opcional)</div>
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                        <input
                          className="rounded-xl bg-black/20 border border-white/10 px-3 py-2 text-sm"
                      ...
                      <div className="mt-2 text-xs text-white/45">
                        Cada vendedor después tendrá su link del evento para contabilizar ventas.
                      </div>
                    </div>
        ...
        {/* PASO 4: Confirmación (términos + guardar final) */}
        {activeTab === "confirm" && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-sm font-semibold text-white mb-1">Confirmación</div>
              <div className="text-xs text-white/60">
                Último paso: aceptá términos y guardá. Esto “publica” el evento (active=true si el backend lo soporta).
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <button
                onClick={() => setActiveTab("sellers")}
                className="px-6 py-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-black uppercase tracking-widest text-[10px]"
              >
                Volver
              </button>
              <button
                onClick={() => saveEvent({ closeAfterSave: true, finalize: true })}
                className="px-6 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest text-[10px]"
              >
                Guardar (publicar)
              </button>
            </div>
...
// -------------------------
  ...
  const saveEvent = async (opts = {}) => {
    const { closeAfterSave = true, finalize = false } = opts || {};
    if (!editFormData) return;

    // En flujo wizard: permitimos guardar "borrador" sin términos para destrabar
    // la creación del evento y luego cargar tickets/vendedores.
    ...
    try {
      ...
      const payload = {
        ...
        // opcional: fecha ISO si existe (no rompe si backend lo ignora)
        date_iso: editFormData.date_iso || editFormData.dateISO || null,
        ...
        // alias donde se acreditan las ventas (backend puede ignorarlo si aún no existe)
        payout_alias: editFormData.payout_alias || editFormData.payoutAlias || null,

        accept_terms: !!editFormData.accept_terms,
        // si el backend soporta active/published, lo prendemos al finalizar
        active: finalize ? true : (editFormData.active ?? false),
      };
      ...
      // Si acabamos de crear, necesitamos dejar el slug/id en el form para habilitar pasos 2/3.
      // El backend suele devolver event/slug. Hacemos best-effort sin romper si no viene.
      const newSlug = data?.event?.slug || data?.slug || payload?.slug || editFormData.slug;
      if (newSlug && editFormData.slug !== newSlug) {
        setEditFormData((p) => ({ ...(p || {}), slug: newSlug, _is_new: false }));
      } else {
        setEditFormData((p) => ({ ...(p || {}), _is_new: false }));
      }
...
{view === "success" && purchaseData && (
      ...
      <GoogleLoginModal
        ...
        onLoggedIn={async (u) => {
          // `u` puede venir "recortado" del modal. Para que el producer panel
          // funcione sin refrescar, siempre re-leemos /api/auth/me.
          setMe(u);
          await refreshMe();
          setLoginRequired(false);
