...
const EditorModal = ({
  ...
  const stepTitles = ["Básico", "Flyer", "Fecha", "Ubicación", "Cobro"];
  const stepSubtitle = [
    "Nombre, slug y términos",
    "Subí el flyer y previsualizalo",
    "Calendario real con hora",
    "Lat/Lng + vista de mapa",
    "Alias de cobro + revisión",
  ];
  ...
  const getStepErrors = (step) => {
    ...
    const dateISO = String(editFormData?.date_iso || "").trim();
    ...
    if (step === 0) {
      ...
      if (!editFormData?.accept_terms) e.push("Tenés que aceptar Términos y Condiciones para continuar.");
    }
    if (step === 1) {
      const flyer = String(editFormData?.flyer_url || "").trim();
      if (!flyer) e.push("Subí un flyer (o pegá un URL) para continuar.");
    }
    if (step === 2) {
      if (!dateISO) e.push("Elegí una fecha en el calendario.");
      if (!time) e.push("Elegí una hora de inicio.");
    }
    if (step === 3) {
      ...
      const la = Number(lat);
      const ln = Number(lng);
      if (!Number.isFinite(la) || la < -90 || la > 90) e.push("Latitud inválida (entre -90 y 90).");
      if (!Number.isFinite(ln) || ln < -180 || ln > 180) e.push("Longitud inválida (entre -180 y 180).");
    }
    if (step === 4) {
      if (!payout) e.push("Falta el alias de cobro (CBU/alias/Mercado Pago).");
      // Chequeo mínimo: 6+ chars
      if (payout && payout.length < 6) e.push("El alias de cobro parece muy corto.");
      const desc = String(editFormData?.description || "").trim();
      if (!desc) e.push("Sumá una descripción breve del evento.");
    }
    return e;
  };
          ...
          <div>
              ...
              {stepTitles.map((t, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      // no permite "saltar" hacia adelante rompiendo flujo
                      if (i > wizardStep) return;
                      setWizardStep(i);
                      setWizardTouched(false);
                    }}
                  ...
                  >
                    <div
                      className={`h-8 w-8 rounded-2xl flex items-center justify-center text-[10px] font-black ${
                        active
            ...
            {wizardStep === 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="text-sm md:col-span-2">
                  <div className="text-white/60 mb-1">Nombre del evento</div>
                  <input
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-3"
                    value={editFormData.title || ""}
                    onChange={(e) => setEditFormData({ ...editFormData, title: e.target.value })}
                    placeholder="Ej: Fiesta in Piazza"
                  />
                </label>

                <label className="text-sm md:col-span-2">
                  <div className="text-white/60 mb-1">Slug (URL corta)</div>
                  <input
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-3"
                ...
                <div className="md:col-span-2 mt-2 flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                  <input
                    type="checkbox"
            ...
            {wizardStep === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="text-sm">
                  <div className="text-white/60 mb-1">Fecha (calendario)</div>
                  <input
                    type="date"
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-3"
                    value={editFormData.date_iso || ""}
                    onChange={(e) => {
                      const dateISO = e.target.value;
                      setEditFormData((p) => ({
                        ...p,
                        date_iso: dateISO,
                        date_text: formatDateText(dateISO, p?.start_time),
                      }));
                    }}
                  />
                </label>
                ...
                <label className="text-sm">
                  <div className="text-white/60 mb-1">Hora inicio</div>
                  <input
                    type="time"
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-3"
                    ...
                    onChange={(e) => {
                      ...
                      setEditFormData((p) => ({
                        ...p,
                        start_time: t,
                        date_text: formatDateText(p?.date_iso, t),
                      }));
                    }}
                  />
                </label>
                ...
                <label className="text-sm md:col-span-2">
                  <div className="text-white/60 mb-1">Fecha visible (auto)</div>
                  <input
                    type="text"
                    className="w-full rounded-xl bg-black/20 border border-white/10 px-3 py-3 text-sm"
                  ...
                  />
                  <div className="text-[11px] text-white/45 mt-1">
                    Guardamos <span className="text-white/70 font-semibold">date_text</span> para compatibilidad con lo que ya tenés.
                  </div>
                </label>
              </div>
            )}
            ...
            {wizardStep === 3 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <label className="text-sm">
                  <div className="text-white/60 mb-1">Venue / Lugar</div>
                  <input
                    className="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-3"
                    value={editFormData.venue || ""}
                    onChange={(e) => setEditFormData({ ...editFormData, venue: e.target.value })}
                    placeholder="Ej: Arena Maipú"
                  />
                </label>
                ...
                <div className="md:col-span-2 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <label className="text-sm md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="Ej: MI.EVENTO.MP (o CBU)"
                    />
                    <div className="text-[11px] text-white/45 mt-1">
                      Esto se usa para liquidaciones / reportes del productor.
                    </div>
                  </label>
                  ...
                  <label className="text-sm md:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Contá qué va a pasar, horarios, restricciones, etc."
                    />
                  </label>
                </div>

                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-xs font-black uppercase tracking-widest text-white/70 mb-3">Resumen</div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-white/80">
                    <div><span className="text-white/50">Evento:</span> {editFormData.title || "—"}</div>
                    <div><span className="text-white/50">Slug:</span> {editFormData.slug || "—"}</div>
                    <div><span className="text-white/50">Fecha:</span> {editFormData.date_text || "—"}</div>
                    <div><span className="text-white/50">Lugar:</span> {(editFormData.city || "—") + " • " + (editFormData.venue || "—")}</div>
                    <div className="md:col-span-2"><span className="text-white/50">Cobro:</span> {editFormData.payout_alias || "—"}</div>
                  </div>
                </div>
              </div>
            )}
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        accept_terms: finalize ? !!editFormData.accept_terms : undefined,
      };

      // En borrador, no enviamos accept_terms para no bloquear por validaciones del backend
      if (!finalize) {
