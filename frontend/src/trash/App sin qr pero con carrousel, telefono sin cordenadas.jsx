...
// Normaliza URLs de assets (flyers/QR/etc) para evitar guardar/mostrar blobs o rutas relativas rotas
const normalizeAssetUrl = (u, opts = {}) => {
  if (!u) return "";
  const s = String(u).trim();
  if (!s) return "";
  const allowBlob = !!opts.allowBlob;

  // "blob:" sirve para preview local (no para persistencia / links finales)
  if (s.startsWith("blob:")) return allowBlob ? s : "";

  // Absolutas (http/https/data) quedan tal cual
  if (/^(https?:)?\/\//i.test(s) || s.startsWith("data:")) return s;

  // Relativas: aseguramos que empiecen con "/"
  return s.startsWith("/") ? s : `/${s}`;
};

function flyerSrc(ev) {
  return normalizeAssetUrl(ev?.flyer_url || ev?.hero_bg || ev?.image_url) || FALLBACK_FLYER;
}
...
const downloadQrPng = async (payload, filename = "qr.png", size = 420) => {
  ...
  const downloadOrderPdf = async (pdfUrl, filename = "tickets") => {
    try {
      ...
      alert("No se pudo descargar el PDF. Probá de nuevo en unos segundos.");
    }
  };
...
function FeaturedCarousel({ events = [], onOpen, formatMoneyFn }) {
  ...
  // desktop: mostramos 1 solo destacado (sin "peek")
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const v = Math.max(0, Math.min(idx, items.length - 1));
      activeRef.current = v;
      setActive(v);
    };
  ...
  // Autoplay (solo si hay más de 1 item). En desktop funciona como "slide".
  useEffect(() => {
...
;
  ...
  const peek = isDesktop ? 0 : 56; // cuánto se ve de la próxima card (desktop: 0)
  const side = 16; // padding lateral
  const cardWidth = wrapW
    ? (isDesktop
        ? Math.max(260, Math.min(900, wrapW - side * 2))
        : Math.max(220, Math.min(520, wrapW - side * 2 - peek)))
    : 320;
  ...
  return (
    <div ref={wrapRef} className="w-full">
      <div
        ref={scrollerRef}
        className={`no-scrollbar flex w-full gap-4 overflow-y-hidden px-4 pb-3 ${isDesktop ? "overflow-x-hidden" : "overflow-x-auto"}`}
        style={{
      ...
      >
        {items.map((ev, idx) => (
          <button
            ...
            style={{
              width: cardWidth,
              scrollSnapAlign: isDesktop ? "center" : "start",
            }}
          >
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Render Google button si hay client_id
    ...
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
      <div className={`w-full max-w-md p-8 rounded-[2.5rem] ${UI.card} text-white`}>
        ...
        <div className="flex gap-2 mb-5">
          <button
            onClick={() => {
              // Si el usuario ya está en "Google", intentamos abrir el prompt (evita "no pasa nada").
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  if (googleClientId && window.google?.accounts?.id) {
                    try {
                      // Abre el flujo de Google (si el botón renderizado no se ve por algún motivo)
...
// Resetea wizard cuando cambia el evento
  ...
  const getStepErrors = (step) => {
    ...
    if (step === 0) {
      ...
      // Coordenadas (lat/lng) dejaron de ser obligatorias en el front.
...
export default function App() {
  // Navegación simple por URL (sin router) para compartir links directos.
  ...
  // Checkout Success (Mercado Pago) - pantalla de confirmación con QR
  const [mpIssuedAssets, setMpIssuedAssets] = useState([]);
  const [mpIssuedReady, setMpIssuedReady] = useState(false);
  const [mpIssuedOrderId, setMpIssuedOrderId] = useState(null);
  const [mpWaitStatus, setMpWaitStatus] = useState("connecting");
  ...
  // Si entran por URL directa (/evento/:slug), abrimos el detalle automáticamente.
  // También habilita "atrás" del navegador para volver al listado.
  useEffect(() => {
    const syncFromPath = () => {
      // Soportar back_urls de Mercado Pago (vienen como hash: #/checkout/success?order_id=...)
      const hash = window.location?.hash || "";
      if (hash.startsWith("#/checkout/")) {
        try {
          const raw = hash.slice(2); // "checkout/success?order_id=..."
          const [routePart, qs = ""] = raw.split("?");
          const route = routePart || "";
          const sp = new URLSearchParams(qs);
          const order_id = sp.get("order_id") || sp.get("external_reference") || sp.get("order") || null;
          const status = sp.get("status") || sp.get("collection_status") || null;
          if (route.includes("failure")) {
            setMpReturn({ route, order_id, status });
            if (view === "detail") {
          ...
          console.warn("MP return parse error", e);
        }
      }

      const path = window.location?.pathname || "/";
      const m = path.match(/^\/evento\/([^\/?#]+)$/);
      if (m && m[1]) {
        const slug = decodeURIComponent(m[1]);
        // Evita recargar si ya estamos en ese mismo evento
        if (selectedEvent?.slug !== slug) {
      ...
      // Cualquier otra ruta -> volvemos a cartelera pública
      if (view === "detail") {
        setSelectedEvent(null);
        setSelectedTicket(null);
        setView("public");
  ...
  // MP return (hash back_urls): esperamos webhook y luego llevamos a Mis Tickets
  const [mpReturn, setMpReturn] = useState({ route: null, order_id: null, status: null });
  const mpPollRef = useRef(null);
  const mpTransferTriedRef = useRef(false);
  const mpLastOrderIdRef = useRef(null);
  ...
  const checkoutErrors = useMemo(() => {
    ...
    if (!phone) errors.phone = "Ingresá tu celular.";
    else if (phone.length < 8) errors.phone = "Celular inválido.";
...
const [selectedTicket, setSelectedTicket] = useState(null);
  ...
  const loadMyAssets = async (orderId) => {
    setMyAssetsLoading(true);
    setMyAssetsError(null);
    try {
      const r = await fetch(`/api/orders/my-assets?tenant=default${orderId ? `&order_id=${encodeURIComponent(orderId)}` : ''}`, { credentials: "include" });
      const data = await readJsonOrText(r);
  ...
  const currentOrderId = mpReturn?.order_id || null;


  // Si volvemos desde Mercado Pago (success/pending), esperamos a que el webhook marque la orden como paid
  // y cree los tickets. Mientras tanto, hacemos polling a /my-assets y cuando aparezcan, vamos a Mis Tickets.
  useEffect(() => {
    const orderId = mpReturn?.order_id;
    if (view !== "mpWait" || !orderId) return;
      // Evita re-ejecutar si ya mostramos los QR de esta orden
      if (mpIssuedReady && String(mpIssuedOrderId || "") === String(orderId)) {
        setMpWaitStatus("ready");
        return;
      }

      // Si es una orden nueva, resetea estados locales
      if (String(mpLastOrderIdRef.current || "") !== String(orderId)) {
        mpLastOrderIdRef.current = String(orderId);
        mpTransferTriedRef.current = false;
        setMpIssuedReady(false);
        setMpIssuedAssets([]);
        setMpIssuedOrderId(orderId);
      }


    let tries = 0;
    const maxTries = 40; // ~100s (2.5s * 40)
    const tick = async () => {
      tries += 1;
      try {
        // Primero: forzamos sync contra MP (por si solo llegó payment.created y no payment.updated)
try {
  await fetch(`/api/payments/mp/sync-order?tenant=default&order_id=${encodeURIComponent(orderId)}`, { credentials: "include" });
} catch (e) {}

const r = await fetch(`/api/orders/my-assets?tenant=default&order_id=${encodeURIComponent(orderId)}`, { credentials: "include" });
        const data = await readJsonOrText(r);
        const assets = (r.ok && data?.ok && Array.isArray(data.assets)) ? data.assets : [];
        setMyAssets(assets);

        const found = assets.some((a) => String(a?.order_id || "") === String(orderId));

        // Si la orden se creó antes de loguearse, intentá asociarla al usuario actual una vez
        if (!found && !mpTransferTriedRef.current && me?.email) {
          mpTransferTriedRef.current = true;
          try {
            await transferOrder({ order_id: orderId, to_email: me.email });
          } catch (e) {}
        }

        if (found) {
          const assetsForThisOrder = assets.filter(
            (a) => String(a?.order_id || "") === String(orderId)
          );

          setMpIssuedOrderId(orderId);
          setMpIssuedAssets(assetsForThisOrder);
          setMpIssuedReady(true);

          // Limpia el hash de Mercado Pago para que al refrescar no repita el flujo
          try {
          ...
          // deja un pequeño delay para que el backend termine de persistir todo
          setTimeout(() => {
            setMpWaitStatus("ready");
          }, 250);
      ...
      if (tries >= maxTries) {
        // Dejamos un fallback: el usuario puede ir a Mis Tickets manualmente
        return;
      }
      mpPollRef.current = window.setTimeout(tick, 2500);
    };
    ...
    return () => {
      try {
        if (mpPollRef.current) window.clearTimeout(mpPollRef.current);
      } catch {}
      mpPollRef.current = null;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, mpReturn?.order_id]);
            ...
            <div className="mt-10">
              ...
              <div className="mt-4">
                <FeaturedCarousel
                  ...
                  onOpen={(ev) => {
                    setQuantity(1);
                    setCheckoutForm({ fullName: "", dni: "", phone: "", address: "", acceptTerms: false });
                  ...
                  {(selectedEvent.description || selectedEvent.address) && (
                    <div className="mb-8">
                      {selectedEvent.description && (
                        <div>
                          <div className="text-[11px] font-black uppercase tracking-widest text-white/60 mb-2">Sobre el evento</div>
                          <div className="text-[12px] text-neutral-300 leading-relaxed whitespace-pre-line">
                            {selectedEvent.description}
                          </div>
                        </div>
                      )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        // Solo números (permite pegar con espacios/guiones)
  ...
  >
    <ShoppingCart size={16} /> Reservar (próximamente)
                  ...
                  <div className="mt-6 p-4 rounded-2xl bg-white/5 border border-white/10 text-[11px] text-neutral-400 leading-relaxed">
                    ...
                    Recibís tus QRs al confirmar el pago. 
                  </div>
                </div>
              </div>
            </div>
...
{/* PRODUCER (demo) */}
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        <button
                          onClick={() => { navigateTo(`/evento/${encodeURIComponent(ev.slug)}`); openPublicEvent(ev.slug); }}
                        ...
                        <div key={`${kind}-${a.id}`} className="rounded-3xl bg-white/5 border border-white/10 overflow-hidden">
                          <div className="flex gap-5 p-5">
                            ...
                            <div className="w-28 h-28 rounded-3xl overflow-hidden bg-white/10 flex-shrink-0">
                              {normalizeAssetUrl(a.flyer_url) ? (
                                <img src={normalizeAssetUrl(a.flyer_url)} alt="" className="w-full h-full object-cover" />
                              ) : (
                            ...
                            <div className="flex flex-col md:flex-row gap-5 items-start md:items-center justify-between">
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-2">
                                  ...
                                  {a.order_id && (a.pdf_url || a.order_id) && (
                                    <button
                                      onClick={() =>
                                        downloadOrderPdf(
                                          a.pdf_url || `/api/tickets/orders/${a.order_id}/pdf?tenant=default`,
                                          (a.event_title || "tickets").toString().slice(0, 40).replace(/\s+/g, "_") + `_${a.order_id}`
                                        )
                                      }
                                      className="px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest bg-white/10 hover:bg-white/15 transition-all border border-white/10 text-white"
                                    >
                                      Descargar PDF
                                    </button>
                                  )}
        ...
        {/* MP WAIT (vuelta desde Mercado Pago) */}
        {view === "mpWait" && (
          <div className="pt-0 pb-20 px-6 max-w-7xl mx-auto animate-in fade-in text-white">
            <div className="max-w-2xl mx-auto">
              <div className={`p-10 rounded-[2.5rem] ${UI.card} text-center`}>

              <div className="text-xs tracking-[0.28em] text-white/50 mb-3">
                CONFIRMACIÓN DE PAGO
              </div>

              {mpIssuedReady && mpIssuedAssets.length > 0 ? (
                <>
                  ...
                  <p className="mt-4 text-white/70 max-w-xl mx-auto">
                    Estas son tus entradas. También quedan guardadas en <b>Mis Tickets</b>.
                  </p>

                  <div className="mt-7 grid gap-4 max-w-2xl mx-auto">
                    {mpIssuedAssets.map((a, i) => (
                      <div
                        key={a.id || a.ticket_id || `${i}-${a.qr_payload || ""}`}
                        className="p-5 rounded-3xl bg-white/5 border border-white/10 text-left"
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-semibold">
                              {a.event_title || a.event_name || a.event || "Entrada"}
                            </div>
                            <div className="text-xs text-white/60 mt-1">
                              {a.ticket_id ? `Ticket #${a.ticket_id}` : a.order_id ? `Orden ${a.order_id}` : ""}
                            </div>
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              ...
                              <button
                                className="px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-sm"
                                onClick={() => {
                                  setView("myTickets");
                                }}
                              >
                ...
                <>
                  <h2 className="text-4xl md:text-5xl font-extrabold leading-tight">
                    Estamos generando tus QRs…
                  </h2>

                  <p className="mt-4 text-white/70 max-w-xl mx-auto">
                    Mercado Pago confirmó la operación. Estamos esperando el OK final y en segundos te mostramos tus QRs.
                  </p>

                  <div className="mt-6 flex items-center justify-center gap-3 text-white/70">
                    <span className="loader" />
                    <span>
                      {mpWaitStatus === "polling" ? "Procesando..." : mpWaitStatus === "timeout" ? "Listo (sin QR aún)" : "Conectando..."}
                      {mpReturn?.order_id ? ` Orden ${String(mpReturn.order_id).slice(0, 8)}...` : ""}
                    </span>
                  </div>
                  ...
                  <p className="mt-6 text-xs text-white/40">
                    Si se demora, tocá “Actualizar”. Las entradas también quedarán disponibles luego en “Mis Tickets”.
                  </p>
                </>
              )}
            ...
            <div className="max-w-3xl mx-auto">
              <div className={`p-10 rounded-[2.5rem] ${UI.card} text-center`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    onClick={() => {
                      const ids = (purchaseData?.tickets || [])
                        .map(t => t?.ticket_id || t?.id)
                        .filter(Boolean)
                        .join(",");
                      if (!ids) {
                        alert("No hay tickets para descargar todavía.");
                        return;
                      }
                      // PDF real generado por backend (requiere sesión)
                      const url = `/api/orders/tickets.pdf?tenant=default&ids=${encodeURIComponent(ids)}`;
                  ...
                  <button
                    onClick={async () => {
                      const ids = (purchaseData?.tickets || [])
                        ...
                        .join(",");
                      if (!ids) {
                        alert("No hay tickets para compartir todavía.");
                        return;
                      }
                      const url = `${window.location.origin}/api/orders/tickets.pdf?tenant=default&ids=${encodeURIComponent(ids)}`;
                      try {
                        if (navigator.share) {
