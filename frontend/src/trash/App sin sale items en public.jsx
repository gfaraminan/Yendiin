...
export default function App() {
  ...
  const [producerOwnedEvents, setProducerOwnedEvents] = useState([]); // eventos del productor autenticado
  const [producerOwnedEventsLoading, setProducerOwnedEventsLoading] = useState(false);
  const [producerOwnedEventsError, setProducerOwnedEventsError] = useState(null);
  ...
  const refreshMe = async () => {
    try {
      ...
      setMe((prev) => {
        // Evita bucles de re-render cuando el backend devuelve el mismo usuario
        // (mismo sub/email/provider/etc.) pero como un objeto nuevo.
        if (!user) return null;
        if (
          prev &&
          prev.sub === user.sub &&
  ...
  const resetProducerState = () => {
    setProducerOwnedEvents([]);
    setProducerOwnedEventsLoading(false);
    setProducerOwnedEventsError(null);
  ...
  // Si cambia el usuario (logout o login con otra cuenta), limpiamos caches del panel producer
  const meKey = useMemo(() => (me ? `${me.email || me.sub || me.id || ""}` : ""), [me]);
  useEffect(() => {
    resetProducerState();
    // Si estamos adentro del producer, el guard se va a encargar de revalidar.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [meKey]);
  ...
  // ------------------------------------------------------------
  // Producer guard: no entrar a /producer sin sesión (doble candado)
  // ------------------------------------------------------------
  const [producerLoginRequestedAt, setProducerLoginRequestedAt] = useState(0);

  const goProducer = async () => {
    const user = await refreshMe();
    if (user) {
      setView("producer");
      return;
    }
    // No navegar. Pedir login Google.
    setProducerLoginRequestedAt(Date.now());
    openLoginModal({ purpose: "producer" });
  };

  // Si por URL/bug se llegó a la vista producer sin sesión (o se deslogueó estando adentro):
  // volvemos a cartelera y pedimos login.
  useEffect(() => {
    if (view !== "producer") return;

    // Si ya sabemos que no hay sesión, no esperes: volvé y pedí login.
    if (!me) {
      setView("public");
      setProducerLoginRequestedAt(Date.now());
      openLoginModal({ purpose: "producer" });
    ...
    // Doble chequeo contra el backend (por si la cookie expiró y el estado quedó viejo)
    (async () => {
      const user = await refreshMe();
      if (user) return;
      resetProducerState();
  ...
  const loadProducerOwnedEvents = async () => {
    setProducerOwnedEventsLoading(true);
    setProducerOwnedEventsError(null);
    try {
      ...
      const mapped = list.map((e) => ({
        ...
        stock_total: e.stock_total ?? 0,
        stock_sold: e.stock_sold ?? 0,
        revenue: e.revenue ?? 0,
        items: Array.isArray(e.items) ? e.items : [],
        sellers: Array.isArray(e.sellers) ? e.sellers : [],
      }));
      setProducerOwnedEvents(mapped);

      if (!selectedProducerEventSlug && mapped.length > 0) {
        setSelectedProducerEventSlug(mapped[0].slug);
      }
    } catch (e) {
      setProducerOwnedEventsError(e?.message || "No se pudieron cargar tus eventos.");
      setProducerOwnedEvents([]);
    } finally {
      setProducerOwnedEventsLoading(false);
    }
...
<div className="hidden md:grid grid-cols-3 gap-8">
              ...
              {[
                {
                  ...
                  val: `$${(producerOwnedEvents.reduce((acc, ev) => acc + (ev.revenue || 0), 0) / 1000).toLocaleString()}k`,
                  icon: DollarSign,
                },
                {
                  label: "Eventos Activos",
                  val: `${producerOwnedEvents.filter((e) => e.active).length}`,
                  icon: CheckCircle2,
                },
                {
                  label: "Tickets Vendidos",
                  val: `${producerOwnedEvents.reduce((acc, ev) => acc + (ev.stock_sold || 0), 0).toLocaleString()}`,
                  icon: Ticket,
                },
                {
                  label: "Sellers",
                  val: `${producerOwnedEvents.reduce((acc, ev) => acc + (ev.sellers?.length || 0), 0)}`,
                  icon: Users,
                },
              ].map((k, idx) => (
              ...
              <div className={`p-8 rounded-[2.5rem] ${UI.card} lg:col-span-2`}>
                <div className="flex items-start justify-between gap-6 mb-8">
                  <div>
                    ...
                    <div className="text-[11px] text-neutral-400 mt-2">
                      Estos son <span className="font-black">solo tus eventos</span> (no los de todos).
                    </div>
                  </div>
                  <button
                    onClick={() => openEditor()}
                ...
                <div className="space-y-4">
                  {producerOwnedEventsLoading && (
                    <div className="p-6 rounded-3xl bg-white/5 border border-white/10 text-[11px] text-neutral-400 flex items-center gap-3">
                      <Loader2 className="animate-spin" size={16} /> Cargando tus eventos…
                    </div>
                  )}
                  {!producerOwnedEventsLoading && producerOwnedEventsError && (
                    <div className="p-6 rounded-3xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-[11px] font-bold">
                      {producerOwnedEventsError}
                    </div>
                  )}
                  {!producerOwnedEventsLoading && !producerOwnedEventsError && producerOwnedEvents.length === 0 && (
                    <div className="p-6 rounded-3xl bg-white/5 border border-white/10 text-[11px] text-neutral-400">
                      No tenés eventos cargados todavía. Creá uno con <span className="font-black">“Nuevo”</span>.
                    </div>
                  )}
                  {!producerOwnedEventsLoading && !producerOwnedEventsError && producerOwnedEvents.map((ev) => (
                    <div
                      key={ev.id || ev.slug}
      ...
      <GoogleLoginModal
        ...
        onLoggedIn={(u) => {
          ...
          const purpose = pendingCheckout?.purpose;
          const method = pendingCheckout?.method;
          setPendingCheckout(null);

          // Si el login fue para entrar al Producer, vamos directo ahí.
          if (purpose === "producer") {
            setView("producer");
            // refrescamos listas propias
            setTimeout(() => {
              loadProducerOwnedEvents();
              loadProducerEvents();
            }, 0);
          ...
          // Si el login fue por checkout, continuamos compra
