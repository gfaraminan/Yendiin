...
const FeaturedCarouselMobile = ({ items = [], onOpen }) => {
  ...
  // Mobile: 1 card full + "peek" of the next card (no page horizontal scroll)
  const GAP = 14; // px between cards
  const PEEK = 56; // px of next card visible (tweak to taste)
  const MIN_CARD = 260;
  ...
  React.useEffect(() => {
    ...
    const compute = () => {
      // el.clientWidth includes padding; our scroller uses px-4 (=16px each side)
      const inner = el.clientWidth;
      const px = Math.max(MIN_CARD, Math.floor(inner - 32 - PEEK));
      setCardPx(px);
    };
      ...
      >
        {items.map((ev) => (
          <button
            ...
            style={{
              width: cardPx ? `${cardPx}px` : "calc(100% - 88px)",
              scrollSnapAlign: "start",
            }}
          >
            <EventCard ev={ev} variant="featured" />
          </button>
        ))}
      </div>
...
// Evita scroll horizontal en mobile (por carrousels o sombras)
useEffect(() => {
  const html = document.documentElement;
  const body = document.body;
  const prevHtml = html.style.overflowX;
  const prevBody = body.style.overflowX;
  const prevMaxW = body.style.maxWidth;
