...
const getPriceCents = (it) => {
  ...
  for (const v of candidates) {
    if (v !== undefined && v !== null && v !== "") {
      ...
      if (Number.isFinite(n)) {
        // Heurística: si viene como "12" probablemente sea unidades, no centavos.
        if (Math.abs(n) < 1000 && String(v).indexOf(".") === -1 && (it.price_cents==null && it.unit_price_cents==null && it.amount_cents==null)) {
  ...
  // precios anidados
  const p0 = Array.isArray(it.prices) ? it.prices[0] : null;
  if (p0) {
    const n = Number(p0.amount_cents ?? p0.price_cents ?? p0.unit_price_cents ?? 0);
    if (Number.isFinite(n)) return Math.round(n);
  }
  const pmap = it.pricing ?? it.price_map ?? null;
  if (pmap && typeof pmap === "object") {
    const first = Object.values(pmap)[0];
    if (first && typeof first === "object") {
      const n = Number(first.amount_cents ?? first.price_cents ?? first.unit_price_cents ?? 0);
      if (Number.isFinite(n)) return Math.round(n);
    }
  }
  return 0;
...
const formatMoneyCents = (cents, currency = "ARS") => {
  const v = Number(cents || 0) / 100;
  // Mantiene el estilo del UI (sin inventar monedas)
...
const EditorModal = ({
  ...
  const createSaleItem = async () => {
    ...
    const price = Number(saleDraft.price || 0);
      ...
      const payload = {
        ...
        name,
        price: Math.round(price * 100) / 100, // backend guarda en centavos
                              ...
                              <div key={it.id} className="flex items-center justify-between gap-3 rounded-xl bg-black/20 border border-white/10 px-3 py-2">
                                <div className="min-w-0">
                                  ...
                                  <div className="text-xs text-white/50">
                                    {it.currency || "ARS"} ${(Number(it.price_cents ?? 0) / 100).toFixed(2)} · stock {it.stock_total ?? "∞"}
                                  </div>
                                </div>
    ...
    try {
      const res = await fetch("/api/orders/create", {
        ...
        body: JSON.stringify({
          ...
          holder_name: checkoutForm.fullName,
          holder_dni: checkoutForm.dni,
          holder_address: checkoutForm.address,
        }),
      });
      ...
      if (!res.ok) {
        throw new Error((data && (data.detail || data.error || data.message)) || raw || "No se pudo crear la orden");
      }
      if (!data?.ok) {
        throw new Error((data && (data.detail || data.error || data.message)) || "No se pudo crear la orden");
      }
                    ...
                    {(selectedEvent.items || []).map((it) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xl font-black text-indigo-400 italic">
                      <div className="text-base font-black italic text-white">{formatMoney((Number(it.price_cents ?? 0) / 100))}</div>
                          </div>
                        </div>
                      </button>
                    ))}
                  ...
                  <div className="flex items-center justify-between gap-4 pt-2">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-black text-indigo-400 italic mt-1">
                        {formatMoney(((Number(selectedTicket?.price_cents ?? 0) / 100) * quantity))}
                      </div>
                    </div>
                  </div>
