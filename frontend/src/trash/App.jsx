...
const EditorModal = ({
  ...
  }, [isOpen, activeTab, eventSlug, tenantId]);
  ...
  const createSaleItem = async () => {
    ...
    try {
      const payload = {
        tenant_id: tenantId,
        event_slug: eventSlug,
        name: (saleDraft.name || "").trim(),
        price: Number(String(saleDraft.price || "0").replace(",", ".")),
        currency: saleDraft.currency || "ARS",
        stock_total:
          saleDraft.stock_total === "" ? null : Number(saleDraft.stock_total),
        sort_order:
          saleDraft.sort_order === "" ? null : Number(saleDraft.sort_order),
      };
      await fetchJson(`/api/producer/sale-items`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const createSeller = async () => {
    ...
    try {
      ...
      await fetchJson(`/api/producer/sellers`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      setSellerDraft({ code: "", name: "" });
      await loadSellers();
    } catch (e) {
      setTabError(String(e.message || e));
    } finally {
