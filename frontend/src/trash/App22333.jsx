...
export default function App() {
  ...
  const myManagedEvents = useMemo(() => {
    const slugs = new Set((producerEvents || []).map((e) => e.event_slug).filter(Boolean));
    return (events || []).filter((ev) => slugs.has(ev.slug));
  }, [events, producerEvents]); // [{ event_slug, orders_count, total_cents, bar_cents, tickets_cents }]
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-xl border-b border-white/5 overflow-x-hidden">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-4 sm:py-5">
          ...
          <nav className="mt-4 flex items-center justify-center sm:justify-start gap-2 w-full">
            ...
            <button
              onClick={() => {
                    if (!me) { setAfterLoginView("producer"); setLoginRequired(true); }
                    else { setView("producer"); }
                  }}
              className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${
              ...
              <div className={`p-8 rounded-[2.5rem] ${UI.card} lg:col-span-2`}>
                ...
                <div className="space-y-4">
                  {myManagedEvents.length === 0 ? (
                    <div className="p-6 rounded-3xl bg-white/5 border border-white/10 text-[12px] text-neutral-300">
                      No tenés eventos propios todavía. Creá uno con <span className="font-bold">Nuevo</span>.
                    </div>
                  ) : (
                    myManagedEvents.map((ev) => (
                      <div
                        key={ev.id}
