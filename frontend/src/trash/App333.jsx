          ...
          window.google.accounts.id.initialize({
            ...
            callback: async (resp) => {
              try {
                ...
                // Si hay flyer seleccionado, lo subimos y actualizamos flyer_url
                const savedSlug = data?.event?.slug || editFormData.slug;
                if (flyerFile && savedSlug) {
                  const fd = new FormData();
                  fd.append('file', flyerFile);
                  const up = await fetch(`/api/producer/events/${savedSlug}/flyer?tenant_id=${tenantId}`, {
                    method: 'POST',
                    credentials: 'include',
                    body: fd,
                  });
                  const upData = await readJsonOrText(up);
                  if (!up.ok || !upData?.ok) throw new Error(upData?.detail || 'No se pudo subir el flyer');
                  // guardamos la URL en el evento
                  const upd = await fetch(`/api/producer/events/${savedSlug}?tenant_id=${tenantId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ flyer_url: upData.url }),
                  });
                  const updData = await readJsonOrText(upd);
                  if (!upd.ok || !updData?.ok) throw new Error(updData?.detail || 'No se pudo guardar la URL del flyer');
                  setFlyerFile(null);
                }
                onLoggedIn({
  ...
  const getStepErrors = (step) => {
    ...
    if (step === 0) {
      ...
      if (!cuit) e.push("Falta el CUIT.");
	      // El checkbox del paso 1 usa `accept_terms` (no `accept_terms`)
