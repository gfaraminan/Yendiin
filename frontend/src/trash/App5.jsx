...
export default function App() {
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      // ✅ Si es un evento nuevo, creamos al menos 1 ticket (sale_item) para que el checkout funcione
      if (!isUpdate) {
        const priceNum = Number(editFormData.ticket_price || 0);
        const priceCents = Number.isFinite(priceNum) ? Math.max(0, Math.round(priceNum * 100)) : 0;

        try {
          await fetch("/api/producer/sale-items/create", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
              event_slug: data.slug,
              name: (editFormData.ticket_name || "Entrada General").trim() || "Entrada General",
              kind: "ticket",
              price_cents: priceCents,
              stock_total: Number(editFormData.ticket_stock || 0) || 0,
              accept_terms: true,
            }),
          });
        } catch (e2) {
          console.warn("No se pudo crear ticket inicial", e2);
        }
      }
