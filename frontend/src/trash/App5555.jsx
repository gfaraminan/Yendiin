...
export default function App() {
  ...
  // Eventos que el productor puede gestionar (intersección entre cartelera pública y su lista de eventos)
  // Esto evita que en la sección "Gestión" se vean eventos ajenos.
  const managedProducerEvents = useMemo(() => {
    const owned = new Set((producerEvents || []).map((e) => e.event_slug));
    return (events || []).filter((ev) => owned.has(ev.slug));
              ...
              <div className={`p-8 rounded-[2.5rem] ${UI.card} lg:col-span-2`}>
                <div className="flex items-start justify-between gap-6 mb-8">
                  <div>
                    ...
                    <div className="text-[11px] text-neutral-400 mt-2">
                      Solo se muestran tus eventos (los que están asociados a tu cuenta de productor).
                    </div>
                  </div>
                  <button
                    onClick={() => openEditor()}
                ...
                <div className="space-y-4">
                  {managedProducerEvents.length === 0 ? (
                    <div className="p-6 rounded-3xl bg-white/5 border border-white/10 text-white/70">
                      No tenés eventos propios todavía. Creá uno con <span className="font-semibold">Nuevo</span>.
                    </div>
                  ) : (
                    managedProducerEvents.map((ev) => (
                    <div
                      key={ev.slug || ev.id}
