...
// PDF bonito (desde backend) + envío por mail
const downloadTicketsPdf = async (tenant, ticketIds = [], filename = "tickets.pdf") => {
  try {
...
const sendTicketsByEmail = async (tenant, ticketIds = []) => {
  try {
    if (!ticketIds || ticketIds.length === 0) return;
    const idsParam = encodeURIComponent(ticketIds.join(","));
...
export default function App() {
  ...
  const orderId = mpReturn?.order_id || null;
...
const [selectedTicket, setSelectedTicket] = useState(null);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      tries += 1;
      try {
        // 1) Sync order con MP (fallback si el webhook se demora)
        try {
          await fetch(`/api/payments/mp/sync-order?tenant=default&order_id=${encodeURIComponent(orderId)}`, {
            credentials: "include",
          });
        } catch (_) {}

        // 2) Consultar assets ya emitidos
          ...
          <nav className="mt-4 flex items-center justify-center sm:justify-start gap-2 w-full">
            ...
            <button
              onClick={() => {
                ...
                setView("myTickets");
                setTimeout(() => {
                  try { loadMyAssets(orderId); } catch (e) {}
                }, 0);
              }}
              className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${
  ...
  >
    <ShoppingCart size={16} /> Reservar (próximamente)
                  ...
                  <div className="mt-6 p-4 rounded-2xl bg-white/5 border border-white/10 text-[11px] text-neutral-400 leading-relaxed">
                    ...
                    Recibiras tus QRs 48 hs antes del evento. 
                  </div>
                </div>
              </div>
            </div>
                          ...
                          <div className="px-5 pb-5">
                            <div className="flex flex-col md:flex-row gap-5 items-start md:items-center justify-between">
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-2">
                                  <button
                                    onClick={() => downloadTicketsPdf(tenant, [a.ticket_id || a.id].filter(Boolean), `ticketpro_${badgeKind.toLowerCase()}_${a.id}.pdf`)}
                                    disabled={!qrPayload}
                                  ...
                                  <button
                                    onClick={() => sendTicketsByEmail(tenant, [a.ticket_id || a.id].filter(Boolean))}
                                    disabled={!qrPayload}
                                    className="px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest bg-white/5 hover:bg-white/10 transition-all border border-white/10 text-white disabled:opacity-40"
                                  >
                                    Enviar mail
          ...
          <div className="pt-0 pb-20 px-6 max-w-7xl mx-auto animate-in fade-in text-white">
            <div className="max-w-2xl mx-auto">
              ...
              {mpIssuedReady && mpIssuedAssets.length > 0 ? (
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              <button
                                ...
                                onClick={() =>
                                  downloadTicketsPdf(
                                    tenant,
                                    [a.ticket_id].filter(Boolean),
                                    `ticket-${a.ticket_id || i + 1}.pdf`
                                  )
                                }
                              >
                                Descargar PDF
                ...
                <>
                  <h2 className="text-4xl md:text-5xl font-extrabold leading-tight">
                    Estamos generando tus entradas, las recibiras 48 hs antes del evento.
                  </h2>

                  <p className="mt-4 text-white/70 max-w-xl mx-auto">
                    Mercado Pago confirmó la operación, pero estamos esperando la validación final y la emisión de tus
                    QR..
                  </p>
