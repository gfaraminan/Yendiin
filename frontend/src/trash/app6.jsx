...
function normalizeFlyerUrl(e) {
  const flyer = (e?.flyer_url || "").trim();
  if (flyer && (flyer.startsWith("http://") || flyer.startsWith("https://") || flyer.startsWith("/"))) return flyer;
  const hero = (e?.hero_bg || "").trim();
  if (!hero) return null;
  if (hero.toLowerCase().startsWith("linear-gradient(")) return null;
