...
def api_producer_events_mine(request: Request, user=Depends(_require_auth)):
    """Alias seguro para listar SOLO eventos del productor logueado.
    (Evita que el front use /api/public/events por error.)"""
...
def api_producer_tickets_summary(
    request: Request,
    tenant_id: str = "default",
    event_slug: str = "",
    user=Depends(_require_auth),
):
    """KPIs de tickets por evento (vendidos/usados/válidos) para el productor owner."""
    tenant_id = _norm_id(tenant_id, default=_tenant_id_from_request(request))
    event_slug = (event_slug or "").strip().lower()
    if not event_slug:
        raise HTTPException(status_code=400, detail="missing_event_slug")
    ...
    with get_conn() as conn:
        ...
        # 1) evento existe y es del producer
        cur.execute(
        ...
        colset = {c[0] if not isinstance(c, dict) else c.get("column_name") for c in cols} if cols else set()
        required = {"tenant_id", "producer_tenant", "event_slug", "status"}
        if not required.issubset(colset):
            raise HTTPException(status_code=500, detail="tickets_schema_invalid")
        ...
        cur.execute(
            ...
            SELECT
              ...
              SUM(CASE WHEN status='valid' THEN 1 ELSE 0 END)::int AS valid
            FROM tickets
            WHERE tenant_id=%s AND producer_tenant=%s AND event_slug=%s
            """,
