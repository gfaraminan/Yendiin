...
async def api_producer_ticket_types_create(req: Request):
    """
    Crea un ticket_type (precio/entrada) en Postgres.
    Soporta:
      - JSON (usado por producer_dashboard_v2.html)
    ...
    price_ars = float(data.get("price_ars") or data.get("price") or 0)
    capacity = int(data.get("capacity") or data.get("stock") or 0)

    if not event_slug or not name:
        # Si vino desde fetch() devolvemos JSON; si vino como form, redirigimos igual.
        if "application/json" in ct:
    ...
    cur = c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    try:
        # Asegura que el evento exista en este tenant
        cur.execute("SELECT 1 FROM events WHERE tenant=%s AND event_slug=%s", (tenant, event_slug))
        if not cur.fetchone():
            c.rollback()
            if "application/json" in ct:
        ...
        cur.execute("""
            INSERT INTO ticket_types (tenant, event_slug, name, price_ars, capacity, is_active, created_at)
            VALUES (%s,%s,%s,%s,%s,TRUE, NOW())
            RETURNING id
        """, (tenant, event_slug, name, price_ars, capacity))
        row = cur.fetchone()
        c.commit()
    finally:
        try: cur.close()
...
def api_producer_ticket_tiers_create(req: Request, payload: dict = Body(...)):
    require_producer(req)
    tenant = (req.session.get("user") or {}).get("tenant") or "demo"
    event_slug = (payload.get("event") or "").strip()
    ...
    price_cents = int(round(float(price_ars) * 100)) if price_ars not in (None, "") else None
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO ticket_type_tiers(event_slug, ticket_type_id, name, price_cents, start_at, end_at)
            VALUES (%s,%s,%s,%s,%s,%s)
            """,
            (event_slug, int(ticket_type_id), name, price_cents, start_at, end_at),
        )
        c.commit()
        return {"ok": True}
    finally:
...
def api_producer_sale_items_create(req: Request, payload: dict = Body(...)):
    ...
    stock_val = int(stock) if str(stock).strip() not in ("", "None") else 0
    sort_val = int(sort_order) if str(sort_order).strip() not in ("", "None") else 0
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO sale_items(event_slug, kind, name, price_cents, stock, start_at, end_at, sort_order, active)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,1)
            """,
            (event_slug, kind, name, price_cents, stock_val, start_at, end_at, sort_val),
        )
        c.commit()

        # Si es una entrada, reflejamos en ticket_types para el checkout (one-source-of-truth práctica)
        if kind.lower() == "entrada":
            cur2 = c.cursor()
            cur2.execute(
                """
                INSERT INTO ticket_types(event_slug, name, price_cents, capacity, active)
                VALUES (%s,%s,%s,0,1)
                ON CONFLICT (event_slug, name) DO UPDATE SET price_cents=EXCLUDED.price_cents, active=1
                """,
                (event_slug, name, price_cents),
            )
            c.commit()
...
def api_producer_sellers_create(req: Request, payload: dict = Body(...)):
    ...
    if _pg_any_enabled():
        ...
        try:
            cur = c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cur.execute("SELECT COALESCE(MAX(id),0)+1 AS next_id FROM public.event_sellers WHERE tenant=%s", (tenant,))
...
def api_producer_reports_summary(req: Request, event: str):
    ...
    if _pg_any_enabled():
        ...
        try:
            cur = c.cursor()
            cur.execute(
                "SELECT COUNT(*), COALESCE(SUM(total_cents),0) FROM public.orders WHERE tenant=%s AND event_slug=%s AND status='PAID'",
                (tenant, event),
            )
            row = cur.fetchone()
        ...
        try:
            ...
            cur.execute("""
                SELECT COALESCE(seller_code,'(sin vendedor)') AS seller_code,
                  ...
                  FROM public.orders
                 WHERE tenant=%s AND event_slug=%s AND status='PAID'
...
def api_ticket_types(req: Request, event_slug: str):
    """
    PG-only. Returns ticket types (and optional tiers) for an event.
    Frontend uses this to decide if the event has prices.
    Tolerant to legacy schemas (price_ars/is_active without capacity).
    """
    tenant = get_tenant(req)
    c = pg_conn()
    try:
        ...
        tt_cols = set(table_columns(cur, "ticket_types").keys())

        price_expr = "COALESCE(price_cents,0) AS price_cents"
        if "price_cents" not in tt_cols and "price_ars" in tt_cols:
            # price_ars is numeric/float; convert to cents
            price_expr = "COALESCE(ROUND(price_ars * 100)::int,0) AS price_cents"
        cap_expr = "COALESCE(capacity,0) AS capacity" if "capacity" in tt_cols else "0 AS capacity"
        active_expr = "COALESCE(active,true) AS active" if "active" in tt_cols else (
            "COALESCE(is_active,true) AS active" if "is_active" in tt_cols else "true AS active"
        )
        order_expr = "COALESCE(sort_order,0), id" if "sort_order" in tt_cols else "id"
        ...
        # tiers (optional)
        tier_cols = set(table_columns(cur, "ticket_type_tiers").keys())
        has_tiers = bool(tier_cols)

        tiers = []
        if has_tiers and ("ticket_type_id" in tier_cols):
            tier_price_expr = "COALESCE(price_cents,0) AS price_cents"
            if "price_cents" not in tier_cols and "price_ars" in tier_cols:
                tier_price_expr = "COALESCE(ROUND(price_ars * 100)::int,0) AS price_cents"

            starts_expr = "starts_at" if "starts_at" in tier_cols else ("start_date" if "start_date" in tier_cols else "NULL::text AS starts_at")
            ends_expr = "ends_at" if "ends_at" in tier_cols else ("end_date" if "end_date" in tier_cols else "NULL::text AS ends_at")
            tier_active_expr = "COALESCE(active,true) AS active" if "active" in tier_cols else (
                "COALESCE(is_active,true) AS active" if "is_active" in tier_cols else "true AS active"
            )

            cur.execute(f"""
                SELECT id, ticket_type_id, name,
                       {tier_price_expr},
                       {starts_expr} AS starts_at,
        ...
        tiers_by_type = {}
        for t in tiers:
            tiers_by_type.setdefault(t.get("ticket_type_id"), []).append(t)
        ...
        for tt in types:
            out.append({
                "id": tt.get("id"),
                "name": tt.get("name"),
                "price_cents": int(tt.get("price_cents") or 0),
                "capacity": int(tt.get("capacity") or 0),
                "active": bool(tt.get("active")) if tt.get("active") is not None else True,
                "tiers": tiers_by_type.get(tt.get("id"), []),
            })
...
def api_event_sale_items(req: Request, event_slug: str):
    """
    PG-only. Returns catalog items (sale items) for an event.
    Used by client menu/checkout to compute totals.
    Tolerant to legacy schemas (different column names).
    """
    tenant = get_tenant(req)
    c = pg_conn()
    try:
        ...
        def pick(cols: set, *names: str):
            for n in names:
                if n in cols:
                    return n
            return None

        # Prefer catalog_items if exists; otherwise fall back to sale_items for legacy.
        cur.execute("SELECT to_regclass(%s) AS reg", ("public.catalog_items",))
        has_catalog = bool((cur.fetchone() or {}).get("reg"))
        table = "catalog_items" if has_catalog else "sale_items"

        cols = set(table_columns(cur, table).keys())

        name_col = pick(cols, "item_name", "name", "title", "product_name")
        type_col = pick(cols, "item_type", "type", "kind", "category")
        order_col = pick(cols, "display_order", "sort_order", "order", "position")
        stock_col = pick(cols, "stock_total", "stock", "qty", "capacity")
        active_col = pick(cols, "active", "is_active", "enabled")

        price_cents_col = pick(cols, "price_cents", "price")
        price_ars_col = pick(cols, "price_ars")

        starts_col = pick(cols, "starts_at", "start_date", "start_at")
        ends_col = pick(cols, "ends_at", "end_date", "end_at")

        # Build safe expressions (never reference missing columns)
        name_expr = f"{name_col} AS item_name" if name_col else "''::text AS item_name"
        type_expr = f"{type_col} AS item_type" if type_col else "''::text AS item_type"
        order_expr = f"COALESCE({order_col},0) AS display_order" if order_col else "0 AS display_order"
        stock_expr = f"COALESCE({stock_col},0) AS stock" if stock_col else "0 AS stock"

        if price_cents_col and price_cents_col != "price":
            price_expr = f"COALESCE({price_cents_col},0) AS price_cents"
        elif price_cents_col == "price":
            # ambiguous legacy: assume "price" is in cents if int-like, otherwise treat as ARS and convert
            price_expr = "COALESCE(price,0) AS price_cents"
        elif price_ars_col:
            price_expr = "COALESCE(ROUND(price_ars * 100)::int,0) AS price_cents"
        else:
            price_expr = "0 AS price_cents"

        active_expr = f"COALESCE({active_col},true) AS active" if active_col else "true AS active"

        starts_expr = f"{starts_col} AS starts_at" if starts_col else "NULL::text AS starts_at"
        ends_expr = f"{ends_col} AS ends_at" if ends_col else "NULL::text AS ends_at"
        ...
        log.error(f"api_event_sale_items error: {e}")
        return JSONResponse([], status_code=200)
    finally:
        try:
            c.close()
...
def api_order_get(req: Request, order_id: str):
    ...
    if _pg_enabled():
        ...
        # seguridad: si tenemos auth_subject en PG, la orden debe pertenecer al buyer
        gs = str(buyer.get("google_sub") or "")
        if gs and row.get("auth_subject") and str(row.get("auth_subject")) != gs:
            raise HTTPException(status_code=404, detail="Orden no encontrada")

        ev_slug = (row.get("event_slug") or "")
        if ev_slug:
            row.update(pg_event_meta(tenant=tenant, slug=ev_slug))

        # item_name best-effort (sin tocar sqlite)
        item_name = None
        try:
            ois = pg_get_order_items(str(row.get("id") or order_id))
            if ois:
                item_name = (ois[0].get("name") or None)
        except Exception:
            pass
        if not item_name:
            ij = row.get("items_json")
            if isinstance(ij, dict):
                lines = ij.get("items") or ij.get("lines") or []
                if isinstance(lines, list) and lines:
                    item_name = (lines[0].get("name") or lines[0].get("title") or None)
        if item_name:
            row["item_name"] = item_name

        # compat: order_id (front)
        row["order_id"] = str(row.get("id") or order_id)

        return row

    if _sqlite_disabled():
        raise RuntimeError("SQLite runtime disabled (SQLITE_DISABLED=1) and Postgres is not enabled for /api/orders/{order_id}")
