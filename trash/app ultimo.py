...
FEATURE_PRODUCER_DASH_V2 = os.getenv("FEATURE_PRODUCER_DASH_V2", "1") == "1"
FEATURE_UNIFIED_EVENT_DETAIL = os.getenv("FEATURE_UNIFIED_EVENT_DETAIL", "1") == "1"
...
# Backward-compatible alias (older routes used this name)
require_producer_session = require_producer


def next_global_id(cur, table_name: str) -> int:
    """Generate a new integer id for tables that don't have a sequence/default.

    NOTE: primary keys are global (id) in this project, so we must not scope by tenant.
    """
    cur.execute(f"SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM public.{table_name}")
    row = cur.fetchone()
    return int(row[0] if isinstance(row, (list, tuple)) else row['next_id'])
...
def view_producer_event_edit(req: Request, slug: str):
    """Compat legacy: editor viejo. En v2 usamos la vista unificada event_detail.
    Safe: si FEATURE_UNIFIED_EVENT_DETAIL=1 redirige al detalle unificado.
    """
    require_producer(req)
    if FEATURE_UNIFIED_EVENT_DETAIL:
        return RedirectResponse(url=f"/productor/eventos/{slug}", status_code=307)
    # Fallback ultra-safe: si no está habilitado, redirigimos al dashboard (sin tocar barra)
    return RedirectResponse(url="/productor/dashboard", status_code=307)
...
@app.get("/api/producer/events/cards")
def api_producer_events_cards(req: Request):
    """Eventos + KPIs rápidos para el dashboard (vendidas + % vendido)."""
    prod = require_producer(req)
    tenant = prod["tenant"]
    ...
    if _pg_any_enabled():
        ...
        try:
            ...
            cur.execute(
                """
                SELECT e.slug, e.title, e.city, e.active, e.date_text, e.venue, e.flyer_url, e.hero_bg
                  FROM events e
                 WHERE e.tenant=%s
                 ORDER BY COALESCE(e.updated_at, e.created_at) DESC NULLS LAST, e.slug ASC
                """,
                (tenant,),
            )
            events = cur.fetchall()

            # capacity total por event = suma capacity de ticket_types (si existe)
            cur.execute(
                """
                SELECT event_slug, COALESCE(SUM(COALESCE(capacity,0)),0) AS cap
                  FROM ticket_types
                 WHERE tenant=%s
                 GROUP BY event_slug
                """,
            ...
            caps = { (r[0] if not isinstance(r, dict) else r.get("event_slug")):
                     (r[1] if not isinstance(r, dict) else r.get("cap")) for r in cur.fetchall() }

            # sold = tickets emitidos (no courtesy)
            cur.execute(
                """
                SELECT it.event_slug, COUNT(*) AS sold
                  FROM issued_tickets it
                 WHERE it.tenant=%s AND COALESCE(it.courtesy,0)=0
                 GROUP BY it.event_slug
                """,
                (tenant,),
            )
            solds = { (r[0] if not isinstance(r, dict) else r.get("event_slug")):
                      (r[1] if not isinstance(r, dict) else r.get("sold")) for r in cur.fetchall() }
        finally:
            c.close()
        ...
        for r in events:
            if isinstance(r, dict):
                ...
                (slug, title, city, active, date_text, venue, flyer_url, hero_bg) = r
                e = dict(slug=slug, title=title, city=city, active=active, date_text=date_text, venue=venue,
                         flyer_url=flyer_url, hero_bg=hero_bg)

            cap = int(caps.get(e["slug"]) or 0)
            sold = int(solds.get(e["slug"]) or 0)
            e["tickets_sold"] = sold
            e["tickets_capacity"] = cap
            e["sold_pct"] = (round((sold / cap) * 100, 2) if cap else None)
            out.append(e)
        return out
    ...
    try:
        ...
        cur.execute(
            """
            SELECT slug, title, city, active, date_text, venue, flyer_url, hero_bg
              FROM events
             WHERE tenant=?
             ORDER BY COALESCE(updated_at, created_at) DESC, slug ASC
            """,
            (tenant,),
        )
        ...
        caps = { r["event_slug"]: int(r["cap"] or 0) for r in cur.fetchall() }

        cur.execute(
            """
            SELECT event_slug, COUNT(*) AS sold
              FROM issued_tickets
             WHERE tenant=? AND COALESCE(courtesy,0)=0
             GROUP BY event_slug
            """,
            (tenant,),
        )
        solds = { r["event_slug"]: int(r["sold"] or 0) for r in cur.fetchall() }
...
async def api_producer_ticket_types_create(req: Request):
    """
    Crea/actualiza un ticket_type (tipo de entrada) en Postgres.

    Acepta JSON (fetch) o form submit (fallback).
    """
    # Compat: en algunos deploys viejos existía require_producer_session()
    require_producer_session(req)
    ...
    name = (data.get("name") or data.get("ticket_name") or "").strip()
    price_cents = int(data.get("price_cents") or 0)
    capacity = int(data.get("capacity") or data.get("stock") or 0)
    active_bool = str(data.get("active") or "true").lower() in ("1", "true", "t", "yes", "y", "on")

    if not event_slug or not name:
        raise HTTPException(status_code=400, detail="Faltan campos: event_slug y/o name")

    conn = pg_conn()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

        cols = set(table_columns(cur, "ticket_types"))
        # Compat tenant vs tenant_id
        tenant_col = "tenant" if "tenant" in cols else ("tenant_id" if "tenant_id" in cols else None)
        if not tenant_col:
            raise HTTPException(status_code=500, detail="DB: ticket_types sin columna tenant/tenant_id")

        # Intento upsert por (tenant,event_slug,name)
        where_cols = [tenant_col, "event_slug", "name"]
        cur.execute(
            f"SELECT id FROM public.ticket_types WHERE {tenant_col}=%s AND event_slug=%s AND name=%s LIMIT 1",
            (tenant, event_slug, name),
        )
        row = cur.fetchone()
        ...
        if existing_id:
            ...
            if set_parts:
                ...
                cur.execute(
                    f"UPDATE public.ticket_types SET {', '.join(set_parts)} WHERE id=%s",
                    tuple(params),
                )
        else:
            new_id = next_global_id(cur, "ticket_types") if "id" in cols else None

            insert_cols = []
            insert_vals = []
            if "id" in cols:
                insert_cols.append("id")
                insert_vals.append(new_id)
            insert_cols += [tenant_col, "event_slug", "name"]
            insert_vals += [tenant, event_slug, name]
            if "price_cents" in cols:
                insert_cols.append("price_cents")
                insert_vals.append(price_cents)
            if "capacity" in cols:
                insert_cols.append("capacity")
                insert_vals.append(capacity)
            if "active" in cols:
                insert_cols.append("active")
                insert_vals.append(active_bool)
            if "created_at" in cols:
                insert_cols.append("created_at")
            ...
            cur.execute(
                f"INSERT INTO public.ticket_types ({', '.join(insert_cols)}) VALUES ({placeholders})",
                tuple(insert_vals),
            )
        ...
        conn.rollback()
        # Siempre JSON (evita el "Unexpected token 'I' ...")
...
def api_producer_sale_items_create(payload: ProducerSaleItemCreate, request: Request):
    """
    Crea/actualiza un ítem del catálogo (entrada / parking / consumición / etc.)
    Usamos la MISMA llave multi-tenant que "barra": columna `tenant` (fallback `tenant_id` si existe).
    """
    require_producer_session(request)
    tenant = (request.session.get("user") or {}).get("tenant") or "demo"

    event_slug = (payload.event_slug or "").strip()
    name = (payload.name or "").strip()
    kind = (payload.kind or "").strip()  # Entrada / Parking / Otro / ...
    price_cents = int(payload.price_cents or 0)
    stock = int(payload.stock or 0)
    start_date = payload.start_date
    end_date = payload.end_date
    order_index = int(payload.order_index or 0)
    active_bool = bool(payload.active) if payload.active is not None else True

    if not event_slug or not name or not kind:
        raise HTTPException(status_code=400, detail="Faltan campos: event_slug, name, kind")

    c = pg_conn_any()
    try:
        cur = c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)

        sale_cols = set(table_columns(cur, "sale_items"))
        tenant_col = "tenant" if "tenant" in sale_cols else ("tenant_id" if "tenant_id" in sale_cols else None)
        if not tenant_col:
            raise HTTPException(status_code=500, detail="DB: sale_items sin columna tenant/tenant_id")

        # Upsert por (tenant,event_slug,name,kind)
        cur.execute(
            f"""SELECT id FROM public.sale_items
                WHERE {tenant_col}=%s AND event_slug=%s AND name=%s AND kind=%s
                LIMIT 1""",
        ...
        def ensure_ticket_type_for_entry():
            # Si el ítem es Entrada, creamos/actualizamos ticket_types para checkout
            if kind.lower() != "entrada":
                return
            tt_cols = set(table_columns(cur, "ticket_types"))
            if not tt_cols:
                return
            tt_tenant_col = "tenant" if "tenant" in tt_cols else ("tenant_id" if "tenant_id" in tt_cols else None)
            if not tt_tenant_col:
                return
            cur.execute(
                f"SELECT id FROM public.ticket_types WHERE {tt_tenant_col}=%s AND event_slug=%s AND name=%s LIMIT 1",
                (tenant, event_slug, name),
            )
            r = cur.fetchone()
            if r:
                set_parts = []
                params = []
                if "price_cents" in tt_cols:
                    set_parts.append("price_cents=%s"); params.append(price_cents)
                if "capacity" in tt_cols:
                    set_parts.append("capacity=%s"); params.append(stock)
                if "active" in tt_cols:
                    set_parts.append("active=%s"); params.append(active_bool)
                if "updated_at" in tt_cols:
                    set_parts.append("updated_at=%s"); params.append(ts)
                if set_parts:
                    params.append(int(r["id"]))
                    cur.execute(f"UPDATE public.ticket_types SET {', '.join(set_parts)} WHERE id=%s", tuple(params))
            else:
                new_tt_id = next_global_id(cur, "ticket_types") if "id" in tt_cols else None
                cols = []
                vals = []
                if "id" in tt_cols:
                    cols.append("id"); vals.append(new_tt_id)
                cols += [tt_tenant_col, "event_slug", "name"]; vals += [tenant, event_slug, name]
                if "price_cents" in tt_cols:
                    cols.append("price_cents"); vals.append(price_cents)
                if "capacity" in tt_cols:
                    cols.append("capacity"); vals.append(stock)
                if "active" in tt_cols:
                    cols.append("active"); vals.append(active_bool)
                if "created_at" in tt_cols:
                    cols.append("created_at"); vals.append(ts)
                if "updated_at" in tt_cols:
                    cols.append("updated_at"); vals.append(ts)
                placeholders = ",".join(["%s"]*len(cols))
                cur.execute(
                    f"INSERT INTO public.ticket_types ({', '.join(cols)}) VALUES ({placeholders})",
                    tuple(vals),
                )
        ...
        if existing_id:
            set_parts = []
            params = []
            if "price_cents" in sale_cols:
                set_parts.append("price_cents=%s"); params.append(price_cents)
            if "stock" in sale_cols:
                set_parts.append("stock=%s"); params.append(stock)
            if "start_date" in sale_cols:
                set_parts.append("start_date=%s"); params.append(start_date)
            if "end_date" in sale_cols:
                set_parts.append("end_date=%s"); params.append(end_date)
            if "order_index" in sale_cols:
                set_parts.append("order_index=%s"); params.append(order_index)
            if "active" in sale_cols:
                set_parts.append("active=%s"); params.append(active_bool)
            ...
            if set_parts:
                params.append(existing_id)
                cur.execute(
                    f"UPDATE public.sale_items SET {', '.join(set_parts)} WHERE id=%s",
                    tuple(params),
                )
        else:
            new_id = next_global_id(cur, "sale_items") if "id" in sale_cols else None
            cols = []
            vals = []
            if "id" in sale_cols:
                cols.append("id"); vals.append(new_id)
            cols += [tenant_col, "event_slug", "name", "kind"]; vals += [tenant, event_slug, name, kind]
            if "price_cents" in sale_cols:
                cols.append("price_cents"); vals.append(price_cents)
            if "stock" in sale_cols:
                cols.append("stock"); vals.append(stock)
            if "start_date" in sale_cols:
                cols.append("start_date"); vals.append(start_date)
            if "end_date" in sale_cols:
                cols.append("end_date"); vals.append(end_date)
            if "order_index" in sale_cols:
                cols.append("order_index"); vals.append(order_index)
            if "active" in sale_cols:
                cols.append("active"); vals.append(active_bool)
            if "created_at" in sale_cols:
                cols.append("created_at"); vals.append(ts)
            if "updated_at" in sale_cols:
                cols.append("updated_at"); vals.append(ts)
            ...
            placeholders = ",".join(["%s"] * len(cols))
            cur.execute(
                f"INSERT INTO public.sale_items ({', '.join(cols)}) VALUES ({placeholders})",
                tuple(vals),
            )

        ensure_ticket_type_for_entry()
...
async def api_producer_sellers_create(request: Request):
    """
    Crea un vendedor (tracking link) para un evento.
    - PK (id) es global en la tabla, así que no podemos calcularlo "por tenant".
    - Devuelve JSON de error (evita el "Unexpected token 'I' ...").
    """
    require_producer_session(request)
    tenant = (request.session.get("user") or {}).get("tenant") or "demo"
    ...
    event_slug = (payload.get("event_slug") or payload.get("event") or request.query_params.get("event") or "").strip()
    code = (payload.get("code") or "").strip()
    name = (payload.get("name") or "").strip()

    if not event_slug or not code or not name:
        raise HTTPException(status_code=400, detail="Faltan campos: event_slug, code, name")

    c = pg_conn_any()
    try:
        cur = c.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cols = set(table_columns(cur, "event_sellers"))

        tenant_col = "tenant" if "tenant" in cols else ("tenant_id" if "tenant_id" in cols else None)
        if not tenant_col:
            raise HTTPException(status_code=500, detail="DB: event_sellers sin columna tenant/tenant_id")

        # Idempotente: si ya existe code en tenant+evento, actualizamos
        cur.execute(
            f"SELECT id FROM public.event_sellers WHERE {tenant_col}=%s AND event_slug=%s AND code=%s LIMIT 1",
        ...
        if row:
            ...
            params = [name]
            if "active" in cols:
                set_parts.append("active=true")
            if "updated_at" in cols:
                set_parts.append("updated_at=%s")
                params.append(ts)
            params.append(seller_id)
            cur.execute(f"UPDATE public.event_sellers SET {', '.join(set_parts)} WHERE id=%s", tuple(params))
            c.commit()
            return {"ok": True, "id": seller_id, "code": code, "updated": True}

        new_id = next_global_id(cur, "event_sellers") if "id" in cols else None
        ...
        if "id" in cols:
            insert_cols.append("id")
            insert_vals.append(new_id)
        insert_cols += [tenant_col, "event_slug", "code", "name"]
        insert_vals += [tenant, event_slug, code, name]
        if "active" in cols:
            insert_cols.append("active")
        ...
        placeholders = ",".join(["%s"] * len(insert_cols))
        cur.execute(
            f"INSERT INTO public.event_sellers ({', '.join(insert_cols)}) VALUES ({placeholders})",
            tuple(insert_vals),
        )
        c.commit()
        return {"ok": True, "id": int(new_id) if new_id is not None else None, "code": code}
    except HTTPException:
        c.rollback()
        raise
    except Exception as e:
...
def api_producer_reports_by_seller(req: Request, event: str):
    ...
    if _pg_any_enabled():
        ...
        try:
            ...
            order_cols = set(table_columns(cur, "orders"))
            tenant_col = "tenant" if "tenant" in order_cols else ("tenant_id" if "tenant_id" in order_cols else None)
            if not tenant_col:
                # sin tenant => devolvemos vacío, no rompemos el dashboard
                return {"items": []}

            seller_col = "seller_code" if "seller_code" in order_cols else None

            if seller_col:
                cur.execute(f"""
                    SELECT COALESCE({seller_col},'(sin vendedor)') AS seller_code,
                           COUNT(*) AS orders,
                           COALESCE(SUM(total_cents),0) AS revenue_cents
                      FROM public.orders
                     WHERE {tenant_col}=%s AND event_slug=%s
                     GROUP BY COALESCE({seller_col},'(sin vendedor)')
                     ORDER BY revenue_cents DESC
                """, (tenant, event))
            else:
                # Si la DB todavía no tiene seller_code, agrupamos todo como "(sin vendedor)"
                cur.execute(f"""
                    SELECT '(sin vendedor)' AS seller_code,
                           COUNT(*) AS orders,
                           COALESCE(SUM(total_cents),0) AS revenue_cents
                      FROM public.orders
                     WHERE {tenant_col}=%s AND event_slug=%s
    ...
    """, (tenant, event))
    rows = cur.fetchall()
    items = [{"seller_code": r[0], "orders": r[1], "revenue_cents": r[2]} for r in rows]
    return {"items": items}
...
def api_event_sale_items(event_slug: str):
    """Catálogo unificado (sale_items) público para el cliente (solo activos)."""
    if _pg_any_enabled():
        c = pg_conn_any()
        try:
            cur = c.cursor()
            cur.execute(
                """
                SELECT id, kind, name, label, price_cents, currency, stock_total, stock_sold, active,
                       start_date, end_date
                  FROM sale_items
                 WHERE event_slug=%s AND COALESCE(active,1)=1
        ...
        out = []
        for r in rows:
            # row tuple mapping depends on cursor; support both tuple and dict (RealDictCursor)
            if isinstance(r, dict):
                d = r
            else:
                (sid, kind, name, label, price_cents, currency, stock_total, stock_sold, active,
                 start_date, end_date) = r
                d = dict(
                    id=sid, kind=kind, name=name, label=label, price_cents=price_cents, currency=currency,
                    stock_total=stock_total, stock_sold=stock_sold, active=active,
                    start_date=start_date, end_date=end_date
                )
            st = int(d.get("stock_total") or 0)
            ss = int(d.get("stock_sold") or 0)
            d["remaining"] = max(0, st - ss) if st else None
            out.append(d)
        return out
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT id, kind, name, label, price_cents, currency, stock_total, stock_sold, active,
                   start_date, end_date
              FROM sale_items
             WHERE event_slug=? AND COALESCE(active,1)=1
             ORDER BY sort_order ASC, id ASC
            """,
            (event_slug,),
        )
...
@app.get("/api/events/{event_slug}/sellers")
def api_event_sellers(event_slug: str):
    """Vendedores activos (público) para selector opcional en compra."""
    if _pg_any_enabled():
        c = pg_conn_any()
        try:
            cur = c.cursor()
        ...
        for r in rows:
            if isinstance(r, dict):
                out.append(dict(code=r.get("code"), name=r.get("name")))
            else:
                out.append(dict(code=r[0], name=r[1]))
        return out
