...
def _pick_col(cur, table: str, *candidates: str) -> str:
    """Devuelve el primer nombre de columna existente en `table` dentro de `candidates`."""
    cols = _table_columns(cur, table)
    for c in candidates:
        if c in cols:
            return c
    # fallback: devolvemos el primero (sirve para errores más claros)
    return candidates[0] if candidates else 'tenant_id'

def _tenant_col(cur, table: str) -> str:
    return _pick_col(cur, table, 'tenant_id', 'tenant')

def _producer_col(cur, table: str) -> str:
    return _pick_col(cur, table, 'producer_id', 'producer')
...
def api_me(request: Request):
    ...
    return JSONResponse({"ok": True, "tenant": {tcol}, "producer": producer, "user": user})
...
def api_producer_event_create(request: Request, payload: EventCreateIn):
    ...
    with get_conn() as conn:
        ...
        tcol = _tenant_col(cur, 'events')
        pcol = _producer_col(cur, 'events')
        slug = base
        i = 2
        while True:
            cur = conn.execute(
                f"""SELECT 1 FROM events WHERE {tcol} = %s AND {pcol} = %s AND slug = %s LIMIT 1""",
                ({tcol}, producer, slug),
            )
            if not cur.fetchone():
                break
...
def _event_update_impl(request: Request, slug: str, payload: EventUpdateIn):
    ...
    with get_conn() as conn:
        ...
        params.extend([{tcol}, producer, slug])

        cur = conn.execute(
            f"""
            UPDATE events
               SET {", ".join(set_parts)}
             WHERE {tcol} = %s AND {pcol} = %s AND slug = %s
         RETURNING slug
            """,
            tuple(params),
        )
...
# Busca esta función dentro de tu archivo producer.py y reemplázala:
    ...
    with get_conn() as conn:
        cur = conn.execute(
            ...
            RETURNING slug
            """,
            (active, now_v, {tcol}, producer, slug),
        )
        row = cur.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="event_not_found")
...
def api_list_sellers(request: Request, event: str = ""):
    ...
    with get_conn() as conn:
        rows = conn.execute(
            f"""
            SELECT id, {tcol}, event_slug, code, name, active, created_at, updated_at
            FROM event_sellers
            WHERE {tcol} = %s AND event_slug = %s
...
def api_seller_create(request: Request, payload: SellerUpsertIn):
    ...
    with get_conn() as conn:
        # upsert por UNIQUE ({tcol},event_slug,code)
        cur = conn.execute(
            """
            INSERT INTO event_sellers ({tcol}, event_slug, code, name, active, created_at, updated_at)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
            ON CONFLICT ({tcol}, event_slug, code)
            DO UPDATE SET name = EXCLUDED.name, active = EXCLUDED.active, updated_at = EXCLUDED.updated_at
            RETURNING id, {tcol}, event_slug, code, name, active, created_at, updated_at
            """,
            ({tcol}, event_slug, code, name, active, now_s, now_s),
        )
        row = cur.fetchone()
        conn.commit()
